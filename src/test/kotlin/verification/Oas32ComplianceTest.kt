...
class Oas32ComplianceTest {
  ...
  fun `Domain Supports Links`() {
    ...
    val response = EndpointResponse(statusCode = "200", links = mapOf("UserLink" to link))
        ...
        Components(
            securitySchemes =
                mapOf(
                    ...
                    "Basic" to SecurityScheme(type = "http", scheme = "basic")))
  ...
  fun `Domain Components holds Schemas and Parameters`() {
    val components =
        Components(
            schemas = mapOf("User" to SchemaDefinition(name = "User", type = "object")),
            parameters =
                mapOf(
                    "limitParam" to
                        EndpointParameter(
                            name = "limit", type = "integer", location = ParameterLocation.QUERY)),
            responses =
                mapOf(
                    "NotFound" to
                        EndpointResponse(statusCode = "404", description = "Entity not found")))
    ...
    val components =
        Components(
            headers =
                mapOf(
                    "X-Rate-Limit" to
                        Header(type = "integer", description = "Request limit", required = true)),
            pathItems =
                mapOf(
                    "UserPath" to
                        PathItem(
  ...
  fun `Domain Components holds MediaTypes and Examples`() {
    val example = ExampleObject(summary = "Sample", serializedValue = "{\"id\":1}")
    val mediaType =
        MediaTypeObject(schema = SchemaProperty("string"), examples = mapOf("sample" to example))
    val requestBody =
        RequestBody(
            description = "Payload",
  ...
  fun `Domain Supports Discriminator Mapping`() {
    val schema =
        SchemaDefinition(
            ...
            discriminator =
                Discriminator(
                    propertyName = "petType", mapping = mapOf("dog" to "#/components/schemas/Dog")))
        ...
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    ...
                    "tags" to
                        SchemaProperty("array", xml = Xml(wrapped = true, name = "tag_list"))))
