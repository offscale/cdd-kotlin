...
class RoundTripTest {
        ...
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    "id" to
                        SchemaProperty(
                            type = "integer", format = "int64", description = "Unique ID"),
  ...
  fun `Dual Type Round Trip (OAS 3_2)`() {
    ...
    val originalSchema =
        SchemaDefinition(
            ...
            properties = mapOf("title" to SchemaProperty(types = setOf("string", "null"))))
    ...
    assertTrue(
        subProp.types.contains("null"), "Should contain null (recovered from Kotlin nullability)")
  }
  ...
  fun `Round Trip preserves examples`() {
    val originalSchema =
        SchemaDefinition(
            ...
            properties = mapOf("id" to SchemaProperty("integer", example = 1)))
    val kotlinFile = dtoGenerator.generateDto("com.test", originalSchema)
    val sourceCode = kotlinFile.text
    val extractedSchemas = dtoParser.parse(sourceCode)
    val extractedSchema = extractedSchemas.first()
  ...
  fun `Array alias Round Trip preserves items`() {
    val originalSchema =
        SchemaDefinition(
            name = "IdList", type = "array", items = SchemaProperty("integer", format = "int64"))
  ...
  fun `Network Endpoint Round Trip preserves structure`() {
    val originalEndpoint =
        EndpointDefinition(
            ...
            parameters = listOf(EndpointParameter("id", "Long", ParameterLocation.PATH)),
            responses =
                mapOf(
                    "200" to
                        EndpointResponse(
                            statusCode = "200", description = "Success", type = "TestUser")),
            requestBodyType = null)

    val kotlinFile =
        networkGenerator.generateApi(
            packageName = "com.test", apiName = "UserApi", endpoints = listOf(originalEndpoint))
    val sourceCode = kotlinFile.text
  ...
  fun `Network Endpoint Round Trip preserves security requirements`() {
    val secured =
        EndpointDefinition(
            ...
            responses =
                mapOf(
                    "200" to
                        EndpointResponse(statusCode = "200", description = "ok", type = "String")),
            security =
                listOf(mapOf("api_key" to emptyList()), mapOf("oauth2" to listOf("read", "write"))))
    ...
    val kotlinFile =
        networkGenerator.generateApi(
            packageName = "com.test", apiName = "SecureApi", endpoints = listOf(secured, open))
  ...
  fun `Network Endpoint Round Trip preserves operation servers`() {
    val originalEndpoint =
        EndpointDefinition(
            ...
            servers =
                listOf(Server(url = "https://override.example.com", description = "Override")),
            responses =
                mapOf(
                    "200" to
                        EndpointResponse(statusCode = "200", description = "ok", type = "Unit")))

    val kotlinFile =
        networkGenerator.generateApi(
            packageName = "com.test", apiName = "ServerApi", endpoints = listOf(originalEndpoint))
