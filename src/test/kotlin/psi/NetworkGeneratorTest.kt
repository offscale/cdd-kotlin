...
class NetworkGeneratorTest {
  ...
  fun `generateApi creates Interface, Class, and Exception`() {
    ...
    assertTrue(
        text.contains("class ApiException(message: String) : Exception(message)"),
  ...
  fun `generateApi serializes path parameters with content`() {
    ...
    assertTrue(
        text.contains(
            "encodePathComponent(serializeContentValue(filter, \"application/json\"), false)"))
  }
    ...
    val endpoints =
        listOf(
            EndpointDefinition(
                ...
                requestBody = RequestBody(description = "raw payload", contentPresent = true),
                responses = mapOf("200" to EndpointResponse("200", type = "String"))))
  ...
  fun `generateApi emits paramRef and responseRef tags`() {
    ...
    val responseRef =
        ReferenceObject(
            ref = "#/components/responses/Ok", summary = "Ref summary", description = "Ref desc")
    val endpoints =
        listOf(
            EndpointDefinition(
                path = "/items",
  ...
  fun `generateApi emits response summaries`() {
    ...
    assertTrue(
        text.contains("@responseSummary 200 Short summary"), "Missing @responseSummary KDoc tag")
  }
  ...
  fun `generateApi emits component metadata tags`() {
    ...
    val metadata =
        OpenApiMetadata(
            componentExamples =
                mapOf("Example1" to ExampleObject(summary = "example", value = mapOf("id" to 1))),
            componentLinks = mapOf("GetUser" to Link(operationId = "getUser")),
            componentCallbacks =
                mapOf(
                    "OnEvent" to
                        Callback.Inline(
            ...
            componentResponses =
                mapOf(
                    "NotFound" to
                        EndpointResponse(statusCode = "NotFound", description = "missing")),
            ...
            componentHeaders =
                mapOf(
                    "X-Rate-Limit" to
                        Header(type = "Int", schema = SchemaProperty(types = setOf("integer")))),
            componentPathItems =
                mapOf(
                    "UserPath" to
                        PathItem(
            ...
            componentSchemas = mapOf("Extra" to SchemaDefinition(name = "Extra", type = "object")),
            componentsExtensions = mapOf("x-component-flag" to true))
    val endpoints =
        listOf(
            EndpointDefinition(
                path = "/users",
                    ...
                    post =
                        EndpointDefinition(
                            ...
                            responses =
                                mapOf(
                                    "200" to
                                        EndpointResponse(statusCode = "200", description = "ok")))))
  ...
  fun `generateApi emits root metadata KDoc`() {
    val metadata =
        OpenApiMetadata(
            ...
            info = Info(title = "Meta API", version = "1.2.3", summary = "Summary"),
            ...
            pathItems = mapOf("/pets" to PathItem(summary = "Pets", description = "All pets")),
    ...
    assertTrue(
        text.contains("https://docs.example.com"), "Missing externalDocs url in @externalDocs")
    ...
    assertTrue(
        text.contains("\"x-paths\":\"paths-ext\""), "Missing paths extension in @pathsExtensions")
    ...
    val endpoints =
        listOf(
            EndpointDefinition(
                ...
                responses = mapOf("200" to EndpointResponse("200", type = "String")),
                extensions = mapOf("x-rate-limit" to 10, "x-flag" to true)))
            ...
            EndpointDefinition(
                ...
                responses =
                    mapOf(
                        "200" to
                            EndpointResponse(
                                statusCode = "200", description = "ok", type = "String"))))
  ...
  fun `generateApi interpolates Path parameters (Default Style)`() {
    ...
    assertTrue(
        text.contains(
            "client.request(\"\$baseUrl/users/${'$'}{encodePathComponent(id.toString(), false)}\")"))
  }
  ...
  fun `generateApi emits server variable helpers`() {
    ...
    val servers =
        listOf(
            Server(
                ...
                variables = mapOf("version" to ServerVariable(default = "v1"))))
    ...
    assertTrue(
        text.contains(
            "private val baseUrl: String = defaultBaseUrl(serverIndex, serverName, serverVariables)"),
  ...
  fun `generateApi emits enum typed server variables when enum is defined`() {
        ...
        listOf(
            Server(
                ...
                variables =
                    mapOf(
                        "env" to
                            ServerVariable(default = "prod", enum = listOf("prod", "staging")))))
    ...
    assertTrue(
        text.contains("\"env\" to env.value"), "ServerVariables.toMap should use enum values")
  }
  ...
  fun `generateApi serializes content parameters for query header and cookie`() {
    ...
    assertTrue(
        text.contains(
            "parameter(\"filters\", serializeContentValue(filters, \"application/json\"))"))
    ...
    val endpoints =
        listOf(
            EndpointDefinition(
                ...
                responses = mapOf("200" to EndpointResponse("200", type = "List<String>")),
                parameters =
                    listOf(EndpointParameter("rawQuery", "String", ParameterLocation.QUERYSTRING))))
  ...
  fun `generateApi serializes query object styles`() {
    ...
    assertTrue(
        text.contains("filters.forEach { (key, value) -> parameter(\"filters[\$key]\", value) }"))
    assertTrue(
        text.contains(
            "parameter(\"color\", color.entries.joinToString(\",\") { \"\${it.key},\${it.value}\" })"))
  }
  ...
  fun `generateApi uses encoded parameters when allowReserved is true`() {
    ...
    assertTrue(
        text.contains("url.encodedParameters.append(\"q\", encodeAllowReserved(q.toString()))"))
    assertTrue(
        text.contains("private fun encodeAllowReserved"), "Missing allowReserved encoder helper")
  }
  ...
  fun `generateApi emits empty value when allowEmptyValue is true`() {
    ...
    assertTrue(
        text.contains("else {\n                parameter(\"q\", \"\")"),
        "Missing allowEmptyValue fallback")
  }
                    ...
                    mapOf(
                        "200" to
                            EndpointResponse(
                                ...
                                links = mapOf("next" to Link(operationId = "listUsers")),
                                content =
                                    mapOf(
                                        "application/json" to
                                            MediaTypeObject(
    ...
    val endpoints =
        listOf(
            EndpointDefinition(
                ...
                responses = mapOf("200" to EndpointResponse("200", type = "List<String>")),
                parameters =
                    listOf(EndpointParameter("rawQuery", "Int", ParameterLocation.QUERYSTRING))))
  ...
  fun `generateApi handles style and explode for parameters`() {
        ...
        listOf(
            EndpointDefinition(
                ...
                parameters =
                    listOf(
                        EndpointParameter(
                            "id", "String", ParameterLocation.PATH, style = ParameterStyle.MATRIX),
                        EndpointParameter(
                            "tags",
                            "List<String>",
                            ParameterLocation.QUERY,
                            explode = false), // Comma sep
                        EndpointParameter(
                            "filter",
                            "List<String>",
                            ParameterLocation.QUERY,
    ...
    assertTrue(
        text.contains(
            "client.request(\"\$baseUrl/users;id=${'$'}{encodePathComponent(id.toString(), false)}\")"),
        "Matrix path generation failed")

    // Query param lists
    assertTrue(
        text.contains("parameter(\"tags\", tags.joinToString(\",\"))"),
        "Tags joinToString comma failed")
    assertTrue(
        text.contains("parameter(\"filter\", filter.joinToString(\"|\"))"),
        "Filter joinToString pipe failed")
  }
  ...
  fun `generateApi handles servers and base URL`() {
    val servers =
        listOf(Server("https://api.v1.com", "Prod"), Server("https://staging.v1.com", "Staging"))
    val endpoints =
        listOf(EndpointDefinition(path = "/info", method = HttpMethod.GET, operationId = "getInfo"))
    ...
    assertTrue(
        text.contains("ServerSpec(url = \"https://api.v1.com\""), "Missing prod server entry")
    assertTrue(
        text.contains("ServerSpec(url = \"https://staging.v1.com\""),
        "Missing staging server entry")
  }

  @Test
  fun `generateApi defaults baseUrl to root when servers missing`() {
    val endpoints =
        listOf(EndpointDefinition(path = "/ping", method = HttpMethod.GET, operationId = "ping"))

    val text = generator.generateApi("com.defaultsrv", "DefaultServerApi", endpoints).text

    assertTrue(
        text.contains("private val baseUrl: String = \"/\""),
        "Base URL should default to '/' when servers are absent")
    assertTrue(
        text.contains("client.request(\"${'$'}baseUrl/ping\")"),
        "Request should use default baseUrl")
  }
        ...
        EndpointDefinition(
            ...
            responses =
                mapOf(
                    "200" to
                        EndpointResponse(
                            statusCode = "200", description = "Success", type = "String"),
                    "404" to
                        EndpointResponse(
                            statusCode = "404", description = "Not Found", type = null)))
        ...
        EndpointDefinition(
            ...
            requestBody =
                RequestBody(
                    required = true,
                    content =
                        mapOf("text/*" to MediaTypeObject(schema = SchemaProperty("string")))))

    val text = generator.generateApi("com.wild", "WildcardApi", listOf(endpoint)).text

    assertTrue(
        text.contains("setBody(body)"), "Body should still be set for wildcard content types")
    assertTrue(
            ...
            parameters =
                listOf(
                    EndpointParameter(
                        ...
                        example =
                            ExampleObject(
                                summary = "Filter", dataValue = mapOf("status" to "active")))))
  ...
  fun `generateApi includes parameter schema and content tags`() {
    val schemaParam =
        EndpointParameter(
            ...
            type = "Int",
            location = ParameterLocation.QUERY,
            schema = SchemaProperty(types = setOf("integer"), minimum = 1.0))
    val contentParam =
        EndpointParameter(
            name = "filter",
            type = "String",
  ...
  fun `generateApi generates Ktor Auth configuration`() {
    val endpoints =
        listOf(
            EndpointDefinition(path = "/secure", method = HttpMethod.GET, operationId = "secureOp"))
    val schemes =
        mapOf(
    ...
    assertTrue(
        text.contains("BearerTokens(accessToken = bearerAuth"), "Missing bearer token loading")
  ...
  fun `generateApi supports oauth2 and openIdConnect security schemes`() {
    ...
    assertTrue(
        text.contains("oidcAuth: OAuthTokens? = null"), "Missing OpenID Connect auth parameter")
    assertTrue(text.contains("bearer {"), "Missing bearer block for OAuth2/OpenID Connect")
    assertTrue(
        text.contains("BearerTokens(accessToken = oauth2Auth.accessToken"),
        "Missing OAuth2 bearer token loading")
    assertTrue(
        text.contains("BearerTokens(accessToken = oidcAuth.accessToken"),
        "Missing OpenID Connect bearer token loading")
  }
  ...
  fun `generateApi emits OAuth flow scaffolding helpers`() {
    val endpoints =
        listOf(
            EndpointDefinition(path = "/secure", method = HttpMethod.GET, operationId = "secureOp"))
    val schemes = mapOf("oauth2Auth" to SecurityScheme(type = "oauth2"))

    val text =
        generator.generateApi("com.auth", "OAuthHelpersApi", endpoints, emptyList(), schemes).text

    assertTrue(text.contains("data class OAuthTokens"), "Missing OAuthTokens model")
    assertTrue(
        text.contains("data class OAuthDeviceCodeResponse"), "Missing device code response model")
    ...
    assertTrue(
        text.contains("suspend fun exchangeAuthorizationCode"),
        "Missing authorization code exchange")
    assertTrue(text.contains("suspend fun refreshToken"), "Missing token refresh")
    assertTrue(
        text.contains("suspend fun requestDeviceCode"), "Missing device authorization request")
  ...
  fun `generateApi supports mutualTLS security schemes`() {
    val endpoints =
        listOf(
            EndpointDefinition(path = "/secure", method = HttpMethod.GET, operationId = "secureOp"))
    val schemes = mapOf("mutualTlsAuth" to SecurityScheme(type = "mutualTLS"))

    val text =
        generator.generateApi("com.auth", "MutualTlsApi", endpoints, emptyList(), schemes).text
    ...
    assertTrue(
        text.contains("mutualTlsAuth: MutualTlsConfig? = null"),
        "Missing mutualTLS config parameter")
    assertTrue(
        text.contains("mutualTlsConfigurer: MutualTlsConfigurer? = null"),
        "Missing mutualTLS configurer parameter")
    assertTrue(text.contains("mutualTlsConfigurer?.let"), "Missing mutualTLS configurer invocation")
    assertTrue(
        text.contains("configurer(this, mutualTlsAuth)"), "Missing mutualTLS config application")
  }
            ...
            requestBody =
                RequestBody(
                    ...
                    content =
                        mapOf(
                            "application/json" to
                                MediaTypeObject(schema = SchemaProperty("string")))))
        ...
        listOf(
            EndpointDefinition(
                ...
                parameters =
                    listOf(
                        EndpointParameter(
                            "tags", "List<String>", ParameterLocation.QUERY, explode = true),
                        EndpointParameter(
                            "codes", "Array<String>", ParameterLocation.QUERY, explode = false))))
  ...
  fun `generateApi handles apiKey locations and unsupported locations`() {
    val endpoints =
        listOf(EndpointDefinition(path = "/auth", method = HttpMethod.GET, operationId = "auth"))
    val schemes =
        mapOf(
    ...
    val text =
        generator.generateApi("com.apikey", "ApiKeyApi", endpoints, emptyList(), schemes).text
  ...
  fun `generateMethodSignature handles empty params`() {
    val endpoint = EndpointDefinition(path = "/ping", method = HttpMethod.GET, operationId = "ping")
            ...
            requestBody =
                RequestBody(
                    content =
                        mapOf(
                            "application/json" to
                                MediaTypeObject(schema = SchemaProperty(types = setOf("integer")))),
                    required = false),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", type = "Unit")))
  ...
  fun `generateApi decodes sequential json responses`() {
    ...
    assertTrue(
        text.contains(
            "decodeSequentialJsonList<Int>(response.bodyAsText(), \"application/jsonl\")"))
                    ...
                    content =
                        mapOf(
                            "application/x-www-form-urlencoded" to
                                MediaTypeObject(schema = SchemaProperty(types = setOf("object")))),
                    required = true),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", type = "Unit")))
  ...
  fun `generateApi omits contentType when request body content is empty`() {
    val endpoint =
        EndpointDefinition(
            ...
            requestBody = RequestBody(content = emptyMap(), required = true),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", type = "Unit")))
  ...
  fun `generateApi emits optional parameters with defaults and null checks`() {
    ...
    val text =
        generator
            .generateApi("com.test", "SearchApi", listOf(queryEndpoint, queryStringEndpoint))
            .text
  ...
  fun `generateApi supports querystring content serialization`() {
    ...
    val file =
        generator.generateApi(
            packageName = "com.example", apiName = "QueryApi", endpoints = listOf(endpoint))
  ...
  fun `generateApi infers ByteArray for binary media types without schema`() {
                ...
                mapOf(
                    "200" to
                        EndpointResponse(
                            statusCode = "200",
                            description = "ok",
                            content = mapOf("application/octet-stream" to MediaTypeObject()))))
    ...
    assertTrue(
        text.contains("response.body<ByteArray>()"), "Expected ByteArray response body decoding")
  }
  ...
  fun `generateApi infers String for form media types without schema`() {
    val endpoint =
        EndpointDefinition(
            ...
            requestBody =
                RequestBody(
                    required = true,
                    content = mapOf("application/x-www-form-urlencoded" to MediaTypeObject())),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", description = "ok")))
    ...
    assertTrue(
        text.contains("encodeFormBody(body"),
        "Expected form body encoding for schema-less form content")
  }
