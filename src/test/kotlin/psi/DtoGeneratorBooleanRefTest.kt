...
class DtoGeneratorBooleanRefTest {
  ...
  fun `generate aliases for boolean schemas`() {
    val anySchema = SchemaDefinition(name = "AnyValue", type = "object", booleanSchema = true)
    val noneSchema = SchemaDefinition(name = "NoValue", type = "object", booleanSchema = false)
  ...
  fun `generate alias for schema ref`() {
    val aliasSchema =
        SchemaDefinition(name = "Alias", type = "object", ref = "#/components/schemas/Target")
