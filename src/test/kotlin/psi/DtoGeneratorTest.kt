...
class DtoGeneratorTest {
    ...
    val schema =
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    "sku" to
                        SchemaProperty(type = "string", description = "The stock keeping unit")))
        ...
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    ...
                    "labels" to
                        SchemaProperty(
                            type = "object", additionalProperties = SchemaProperty("string"))))
  ...
  fun `generateDto creates typealias for array schema`() {
    val schema =
        SchemaDefinition(
            name = "Ids", type = "array", items = SchemaProperty("integer", format = "int64"))
  ...
  fun `generateDto includes SerialName annotation`() {
    val schema =
        SchemaDefinition(
            name = "SnakeCase",
            type = "object",
            properties = mapOf("user_id" to SchemaProperty("integer")))
  ...
  fun `generateDto emits advanced schema tags`() {
    val schema =
        SchemaDefinition(
            ...
            contentSchema =
                SchemaProperty(
                    types = setOf("object"), properties = mapOf("id" to SchemaProperty("integer"))),
            properties =
                mapOf(
                    "payload" to
                        SchemaProperty(
                            ...
                            propertyNames =
                                SchemaProperty(types = setOf("string"), pattern = "^[a-z]+$"),
                            dependentRequired = mapOf("a" to listOf("b")),
                            dependentSchemas =
                                mapOf(
                                    "meta" to
        ...
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    "name" to
                        SchemaProperty(
                            type = "string", customKeywords = mapOf("custom" to listOf("a", "b")))))
  ...
  fun `generateDto creates Enum class with SerialName mappings`() {
    val schema =
        SchemaDefinition(
            name = "SortArgs", type = "string", enumValues = listOf("ascending", "descending"))
  ...
  fun `generateDto uses typealias for non-string enums and preserves values`() {
    val schema = SchemaDefinition(name = "Level", type = "integer", enumValues = listOf(1, 2, 3))

    val text = generator.generateDto("com.enums", schema).text

    assertTrue(
        text.contains("typealias Level = Int"), "Non-string enums should not generate enum class")
  ...
  fun `generateDto creates Data Class with inheritance`() {
    val schema =
        SchemaDefinition(
            ...
            properties = mapOf("breed" to SchemaProperty("string")))
  ...
  fun `generateDto includes examples in KDoc`() {
    val schema =
        SchemaDefinition(
            ...
            properties = mapOf("name" to SchemaProperty(type = "string", example = "John")))
  ...
  fun `generateDto includes examples list in KDoc`() {
    val schema =
        SchemaDefinition(
            name = "ExampleList", type = "object", examplesList = listOf("alpha", "beta"))
  ...
  fun `generateDto includes property examples list`() {
    val schema =
        SchemaDefinition(
            ...
            type = "object",
            properties =
                mapOf("count" to SchemaProperty(types = setOf("integer"), examples = listOf(1, 2))))
  ...
  fun `generateDto creates sealed interface without discriminator`() {
    val schema =
        SchemaDefinition(
            ...
            properties = mapOf("kind" to SchemaProperty("string")))
  ...
  fun `generateDto sanitizes empty enum values to Unknown`() {
    val schema = SchemaDefinition(name = "EdgeEnum", type = "string", enumValues = listOf("!!!"))
  ...
  fun `generateDto supports OAS 3_2 Dual Types (array null)`() {
    val schema =
        SchemaDefinition(
            ...
            // Even if required list includes nullableString, type: ["null"] forces nullability in
            // Kotlin
    ...
    assertTrue(
        text.contains("val nullableString: String? = null"), "Should be nullable due to dual type")
    ...
    val schema =
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    "id" to
                        SchemaProperty(type = "string", xml = Xml(attribute = true, name = "pid")),
                    "tags" to
                        SchemaProperty(type = "array", xml = Xml(wrapped = true, name = "tags"))))
