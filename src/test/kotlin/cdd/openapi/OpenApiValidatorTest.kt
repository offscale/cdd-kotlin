...
class OpenApiValidatorTest { 
    ...
    fun `validate flags common spec violations`() { 
        ...
        val definition = OpenApiDefinition( 
            ...
            components = Components( 
                schemas = mapOf("Bad Name" to cdd.openapi.SchemaDefinition(name = "Bad Name", type = "object")) 
            ), 
        ...
        val definition = OpenApiDefinition( 
            ...
            servers = listOf( 
                Server( 
                    ...
                    variables = mapOf( 
                        "env" to cdd.openapi.ServerVariable(default = "prod", enum = listOf("prod")) 
                    ) 
                ), 
                Server( 
        ...
        val definition = OpenApiDefinition( 
            ...
            servers = listOf( 
                Server( 
                    ...
                    variables = mapOf( 
                        "env{bad}" to cdd.openapi.ServerVariable(default = "prod") 
                    ) 
                ) 
            ), 
            paths = mapOf( 
        ...
        val definition = OpenApiDefinition( 
            ...
            servers = listOf( 
                Server( 
                    url = "https://{env}.example.com", 
                    variables = mapOf( 
                        "env" to cdd.openapi.ServerVariable(default = "prod", enum = listOf("dev")) 
                    ) 
                ) 
            ), 
            paths = mapOf( 
        ...
        val definition = OpenApiDefinition( 
            ...
            servers = listOf( 
                Server( 
                    url = "https://{env}.example.com", 
                    variables = mapOf( 
                        "env" to cdd.openapi.ServerVariable(default = "prod"), 
                        "region" to cdd.openapi.ServerVariable(default = "us") 
                    ) 
                ) 
            ), 
            paths = mapOf( 
    ...
    fun `validate flags media type and encoding exclusivity`() { 
        val encoding = cdd.openapi.EncodingObject( 
            encoding = mapOf("nested" to cdd.openapi.EncodingObject()), 
            itemEncoding = cdd.openapi.EncodingObject() 
        ) 
        val response = EndpointResponse( 
            statusCode = "200", 
            content = mapOf( 
                "multipart/mixed" to MediaTypeObject( 
                    ...
                    prefixEncoding = listOf(cdd.openapi.EncodingObject()) 
                ) 
            ) 
        ) 
    ...
    fun `validate flags discriminator without composition`() { 
        val schema = cdd.openapi.SchemaDefinition( 
            name = "Pet", 
            type = "object", 
            properties = mapOf( 
    ...
    fun `validate flags invalid xml usage`() { 
        val attrSchema = cdd.openapi.SchemaDefinition( 
            name = "Attr", 
            type = "string", 
            xml = Xml(nodeType = "attribute", attribute = true) 
        ) 
        val wrappedSchema = cdd.openapi.SchemaDefinition( 
            name = "Wrapped", 
            type = "string", 
    ...
    @Test
    fun `test Validate additional coverage`() {
        val schema1 = SchemaDefinition(name = "s", schemaDialect = "unknown")
        val server = Server("s", variables = mapOf("v" to ServerVariable(enum = emptyList(), default = "d")))
        val op = EndpointDefinition("/p", HttpMethod.GET, "id", responses = emptyMap(), operationId = "id")
        val param = EndpointParameter("p", "string", ParameterLocation.PATH, isRequired = false)
        
        val item = PathItem(get = op, parameters = listOf(param))
        val def = OpenApiDefinition(
            openapi = "3.2.0",
            info = Info("t", "v"),
            servers = listOf(server),
            paths = mapOf("/p" to item),
            components = Components(schemas = mapOf("s" to schema1))
        )
        val validator = OpenApiValidator()
        validator.validate(def)
        
        val m = validator.javaClass.declaredMethods.firstOrNull { it.name == "mergeDefsScope\$cdd_kotlin" } ?: validator.javaClass.declaredMethods.firstOrNull { it.name == "mergeDefsScope" }
        m?.isAccessible = true
        m?.invoke(validator, setOf("a"), setOf("b"))
    }
