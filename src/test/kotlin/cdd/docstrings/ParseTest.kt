...
class OpenApiMetadataTest {
    ...
    @Test
    fun `test OpenApiDefinition toMetadata branches`() {
        val def = cdd.openapi.OpenApiDefinition(info=cdd.openapi.Info("",""))
        val meta = def.toMetadata(includeComponents = false, includePathItems = false, includeWebhooks = false, includeComponentSchemas = false)
        assertNotNull(meta)
    }
