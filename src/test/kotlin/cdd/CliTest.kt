...
class CliTest {
    ...
    fun `test from_openapi generates scaffold`(@TempDir tempDir: Path) {
        ...
        CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("from_openapi", "to_sdk", "-i", inArg, "-o", outArg))
        ...
        CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("to_openapi", "-f", generatedSrc.absolutePath, "--format", "json"))
        ...
        CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", inArg, "-d", generatedSrc.absolutePath))
        ...
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", inArg, "-d", "does_not_exist"))
        } catch (e: Exception) {}

        // Cover !srcDir.isDirectory
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", inArg, "-d", inArg))
        } catch (e: Exception) {}
        ...
        CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", allMethodsSpec.absolutePath, "-d", dummySrc.absolutePath))

        // Trigger top-level exception by passing a directory as spec file
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", generatedSrc.absolutePath, "-d", generatedSrc.absolutePath))
        } catch (e: Exception) {}
    }
    ...
    fun `test from_openapi generates scaffold fallback`(@TempDir tempDir: Path) {
        ...
        val inArg = input.absolutePath
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("from_openapi", "to_sdk", "-i", inArg, "-o", tempDir.toFile().absolutePath))
        } catch (e: Exception) {}
    }
    ...
    fun `test to_openapi fails gracefully on bad input`(@TempDir tempDir: Path) {
        ...
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("to_openapi", "-f", badFile.absolutePath, "--format", "json"))
        } catch (e: Exception) {}
        
        val emptyDir = File(tempDir.toFile(), "empty")
        emptyDir.mkdirs()
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("to_openapi", "-f", emptyDir.absolutePath, "--format", "yaml"))
        } catch (e: Exception) {}
    }
    ...
    fun `test merge_openapi fails gracefully on bad input`(@TempDir tempDir: Path) {
        ...
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", badSpec.absolutePath, "-d", badDir.absolutePath))
        } catch (e: Exception) {}
        
        val goodSpec = File(tempDir.toFile(), "spec.json")
        goodSpec.writeText("{}")
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", goodSpec.absolutePath, "-d", badDir.absolutePath))
        } catch (e: Exception) {}
        
        val goodDir = File(tempDir.toFile(), "src")
        goodDir.mkdirs()
        try {
            CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("merge_openapi", "-s", goodSpec.absolutePath, "-d", goodDir.absolutePath))
        } catch (e: Exception) {}
    }
    ...
    fun `test to_docs_json generates expected json`(@TempDir tempDir: Path) {
        val input = File(tempDir.toFile(), "spec.json")
        input.writeText("{}")
        CddKotlin().subcommands(FromOpenApi().subcommands(ToSdk(), ToSdkCli(), ToServer()), ToOpenApi(), MergeOpenApi(), ToDocsJson()).parse(arrayOf("to_docs_json", "--no-imports", "--no-wrapping", "-i", input.absolutePath))
    }
