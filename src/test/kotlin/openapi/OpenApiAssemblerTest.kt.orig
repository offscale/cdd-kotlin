...
class OpenApiAssemblerTest {
    ...
    val baseComponents =
        Components(
            ...
            schemas =
                mapOf(
                    "Pet" to
                        SchemaDefinition(name = "Pet", type = "object", description = "override")))
  ...
  fun `assemble creates components when schemas provided and no base components`() {
    ...
    val definition = OpenApiAssembler().assemble(info = Info("Pets API", "1.0"), schemas = schemas)
  ...
  fun `assemble merges path item metadata into built paths`() {
    ...
    val pathItems =
        mapOf(
            ...
            "/other" to PathItem(summary = "Other", extensions = mapOf("x-other" to true)))

    val definition =
        OpenApiAssembler()
            .assemble(info = Info("Pets API", "1.0"), endpoints = endpoints, pathItems = pathItems)
  ...
  fun `assemble uses path item ref when provided`() {
    ...
    val pathItems = mapOf("/pets" to PathItem(ref = "#/components/pathItems/Pets"))
        ...
        OpenApiAssembler()
            .assemble(
                ...
                endpoints =
                    listOf(
                        EndpointDefinition(
                            path = "/pets", method = HttpMethod.GET, operationId = "listPets")),
                pathItems = mapOf("/pets" to PathItem(extensions = mapOf("x-new" to "value"))))
