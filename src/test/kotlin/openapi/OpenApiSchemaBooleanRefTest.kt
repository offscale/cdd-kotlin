...
class OpenApiSchemaBooleanRefTest {
    ...
    val components =
        Components(
            schemas =
                mapOf(
                    "AnyValue" to
                        SchemaDefinition(name = "AnyValue", type = "object", booleanSchema = true),
                    "NoValue" to
                        SchemaDefinition(name = "NoValue", type = "object", booleanSchema = false),
                    "Numeric" to
                        SchemaDefinition(
                            name = "Numeric",
                            type = "number",
                    ...
                    "Alias" to
                        SchemaDefinition(
                            name = "Alias", type = "object", ref = "#/components/schemas/Numeric")))

    val definition =
        OpenApiDefinition(info = Info(title = "Test", version = "1.0"), components = components)
  ...
  fun `boolean schema definitions can omit explicit type`() {
    val components =
        Components(
            schemas =
                mapOf("AnyValue" to SchemaDefinition(name = "AnyValue", booleanSchema = true)))
