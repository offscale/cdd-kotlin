...
class OpenApiSchemaKeywordsTest {
  ...
  fun `write supports advanced json schema keywords`() {
    ...
    assertEquals(
        "#/components/schemas/Other", advanced["properties"]["dynamic"]["\$dynamicRef"].asText())
    ...
    assertEquals(
        "country", advanced["dependentSchemas"]["shipping_address"]["required"].first().asText())
  ...
  fun `write preserves custom schema keywords`() {
    val schema =
        SchemaDefinition(
            name = "Custom",
            type = "object",
            customKeywords = mapOf("vendorKeyword" to "alpha", "threshold" to 3),
            properties =
                mapOf(
                    "name" to
                        SchemaProperty(
