...
class OpenApiWriterTest {
  ...
  @Test
  fun `resolveFormat defaults to JSON for unknown extension`() {
    val definition = OpenApiDefinition(info = Info("Format", "1.0"), paths = emptyMap())
    val out = OpenApiWriter().write(definition, OpenApiWriter.Format.AUTO, "file.txt")
    assertTrue(out.startsWith("{"))
    assertTrue(out.contains("\"openapi\""))
  }
  ...
  fun `writeJson omits header style and explode when content is used`() {
    ...
    val response =
        EndpointResponse(
            statusCode = "200", description = "ok", headers = mapOf("X-Test" to header))
    val endpoint =
        EndpointDefinition(
            path = "/headers",
  ...
  fun `writeJson preserves explicit empty response content`() {
    ...
    val response =
        EndpointResponse(
            statusCode = "204", description = "no content", type = "Pet", contentPresent = true)
    val endpoint =
        EndpointDefinition(
            path = "/ping",
            method = HttpMethod.GET,
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Ping", "1.0"), paths = mapOf("/ping" to PathItem(get = endpoint)))
    ...
    val schema =
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    "level" to
                        SchemaProperty(types = setOf("integer"), enumValues = listOf(1, 2, 3)),
                    "flag" to
                        SchemaProperty(types = setOf("boolean"), enumValues = listOf(true, false)),
                    "config" to
                        SchemaProperty(
                            types = setOf("object"),
  ...
  fun `writeJson includes oauth flow scopes even when empty`() {
    ...
    val node = ObjectMapper(JsonFactory()).readTree(json)
    val scopesNode =
        node["components"]["securitySchemes"]["OAuth"]["flows"]["authorizationCode"]["scopes"]
  ...
  fun `writeJson serializes inline schema compositions`() {
    ...
    val schema =
        SchemaDefinition(
            ...
            anyOfSchemas = listOf(SchemaProperty(types = setOf("boolean"))),
            allOfSchemas = listOf(SchemaProperty(types = setOf("object"))))
    val definition =
        OpenApiDefinition(
  ...
  fun `writeJson serializes schema examples list`() {
    val writer = OpenApiWriter()
    val schema =
        SchemaDefinition(name = "Example", type = "object", examplesList = listOf("alpha", "beta"))
    val definition =
        OpenApiDefinition(
  ...
  fun `writeJson uses self as base for component refs`() {
    val writer = OpenApiWriter()
    val petSchema = SchemaDefinition(name = "Pet", type = "object", oneOf = listOf("Cat"))
    val catSchema = SchemaDefinition(name = "Cat", type = "object")
    val requestBodySchema = SchemaDefinition(name = "PetCreate", type = "object")
    val response = EndpointResponse(statusCode = "204", description = "No content", type = "Pet")
    val operation =
        EndpointDefinition(
            path = "/pets",
            method = HttpMethod.GET,
    ...
    val requestBodyRef =
        node["paths"]["/pets"]["get"]["requestBody"]["content"]["application/json"]["schema"][
                "${'$'}ref"]
            .asText()
    assertEquals("https://example.com/openapi#/components/schemas/PetCreate", requestBodyRef)

    val responseRef =
        node["paths"]["/pets"]["get"]["responses"]["204"]["content"]["application/json"]["schema"][
                "${'$'}ref"]
            .asText()
  ...
  private fun buildFullDefinition(): OpenApiDefinition {
    val example =
        ExampleObject(
            summary = "Example", description = "Example description", dataValue = mapOf("id" to 1))
    ...
    val fallbackResponse =
        EndpointResponse(statusCode = "204", description = "No content", type = "Pet")
    val refParameter =
        EndpointParameter(
            name = "refParam",
            type = "String",
    ...
    val refHeader =
        Header(
            ...
            reference =
                ReferenceObject(
                    ref = "#/components/headers/X-Trace", description = "Ref header desc"))
    val refRequestBody =
        RequestBody(
            reference =
                ReferenceObject(
                    ref = "#/components/requestBodies/CreatePet", description = "Ref body desc"))
    val refResponse =
        EndpointResponse(
        ...
        SchemaDefinition(
            ...
            properties =
                mapOf(
                    ...
                    "status" to
                        SchemaProperty(
                            types = setOf("string"), enumValues = listOf("active", "disabled")),
                    "meta" to
                        SchemaProperty(
                            types = setOf("object"),
            ...
            writeOnly = true,
            externalDocs =
                ExternalDocumentation(url = "https://example.com/docs", description = "Docs"),
            discriminator =
                Discriminator(
                    propertyName = "petType", mapping = mapOf("cat" to "#/components/schemas/Cat")),
            xml =
                Xml(
                    name = "pet",
    ...
    val components =
        Components(
            schemas =
                mapOf("Pet" to schema, "User" to SchemaDefinition(name = "User", type = "object")),
            responses =
                mapOf(
                    "Ok" to response,
                    "NoContent" to fallbackResponse,
                    "RefResponse" to refResponse),
            parameters =
                mapOf(
            ...
            examples = mapOf("Pet" to example, "RefExample" to refExample),
            links =
                mapOf(
                        ...
                        PathItem(
                            get =
                                EndpointDefinition(
                                    ...
                                    method = HttpMethod.GET,
                                    operationId = "listPets"))),
            mediaTypes = mapOf("application/json" to mediaType, "text/plain" to refMediaType))
    ...
    val operation =
        EndpointDefinition(
            ...
            externalDocs =
                ExternalDocumentation(url = "https://example.com/op", description = "Op docs"),
            tags = listOf("pets"),
    ...
    val customOperation =
        operation.copy(method = HttpMethod.CUSTOM, customMethod = "COPY", operationId = "copyPets")
    ...
    return OpenApiDefinition(
            ...
            mapOf(
                "/pets" to
                    PathItem(
                        ...
                        additionalOperations = mapOf("COPY" to customOperation),
        ...
        webhooks =
            mapOf(
                "onPet" to
                    PathItem(
                        post = postOperation.copy(path = "/webhook", method = HttpMethod.POST))),
        webhooksExtensions = mapOf("x-webhooks" to mapOf("flag" to true)),
        components = components,
        security = listOf(mapOf("ApiKey" to emptyList())),
        tags = listOf(Tag(name = "pets", summary = "Pets", kind = "nav")),
        externalDocs =
            ExternalDocumentation(url = "https://example.com/external", description = "External"),
        self = "https://example.com/openapi",
  ...
  fun `writeDocument outputs OpenAPI document`() {
    val definition = OpenApiDefinition(info = Info("Doc", "1.0"), paths = emptyMap())
  ...
  fun `write omits requestBody content when content was absent`() {
    val endpoint =
        EndpointDefinition(
            ...
            method = HttpMethod.POST,
            operationId = "submit",
            requestBody = RequestBody(description = "raw payload", contentPresent = false),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", description = "ok")))
    val definition =
        OpenApiDefinition(
