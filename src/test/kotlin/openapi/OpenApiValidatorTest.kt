...
class OpenApiValidatorTest {
  ...
  fun `validate returns empty for minimal valid definition`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"), paths = mapOf("/pets" to PathItem(get = endpoint)))
  ...
  fun `validate accepts explicit empty paths for root presence`() {
    val definition =
        OpenApiDefinition(
            info = Info("Empty Paths", "1.0.0"), paths = emptyMap(), pathsExplicitEmpty = true)

    val issues = OpenApiValidator().validate(definition)

    assertTrue(
        issues.none { it.message.contains("at least one of: paths, webhooks, or components") })
  }
  ...
  fun `isValidJsonPointer rejects invalid pointers`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("JSON Pointer", "1.0"), paths = mapOf("/ev" to PathItem(post = endpoint)))
    val issues = validator.validate(definition)
    val messages = issues.map { it.message }
    // Our runtime-expression check correctly fails bad pointers
    println(messages)
    assertTrue(
        messages.any { it.contains("invalid runtime expression '\$request.body#bad/pointer'") })
    println(messages)
    assertTrue(
        messages.any { it.contains("invalid runtime expression '\$request.body#/bad~2escape'") })
  }
  ...
  fun `validate resolves dynamicRef when anchor exists`() {
    val schema =
        SchemaDefinition(
            name = "Box",
            type = "object",
            defs = mapOf("Payload" to SchemaProperty(type = "string", dynamicAnchor = "payload")),
            properties = mapOf("value" to SchemaProperty(dynamicRef = "#payload")),
  ...
  fun `validate warns when dynamicRef has no anchor in scope`() {
    val schema =
        SchemaDefinition(
            name = "Box",
            type = "object",
            properties = mapOf("value" to SchemaProperty(dynamicRef = "#missing")))
    val definition =
        OpenApiDefinition(
            info = Info("Dynamic", "1.0"),
            components = Components(schemas = mapOf("Box" to schema)))
  ...
  fun `validate flags querystring conflict across path and operation`() {
    val pathQueryString =
        EndpointParameter(
            ...
            content =
                mapOf(
                    "application/json" to
                        MediaTypeObject(schema = SchemaProperty(types = setOf("string")))))
    val opQuery =
        EndpointParameter(
            name = "q",
            type = "String",
    ...
    val definition =
        OpenApiDefinition(
            ...
            paths =
                mapOf("/search" to PathItem(parameters = listOf(pathQueryString), get = endpoint)))

    val issues = validator.validate(definition)
    val messages = issues.map { it.message }
    println(messages)
    assertTrue(
        messages.any { it.contains("query and querystring parameters cannot be used together") })
  }
  ...
  fun `validate accepts callback url with embedded runtime expression`() {
    val callback =
        Callback.Inline(
            expressions =
                mapOf("https://example.com/hook?target={\$request.body#/url}" to PathItem()))
    val endpoint =
        EndpointDefinition(
            path = "/events",
            method = HttpMethod.POST,
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Events", "1.0"), paths = mapOf("/events" to PathItem(post = endpoint)))
  ...
  fun `validate flags invalid header tokens in parameters and responses`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Headers", "1.0"), paths = mapOf("/headers" to PathItem(get = endpoint)))
            ...
            components =
                Components(
                    schemas =
                        mapOf(
                            "Bad Name" to
                                domain.SchemaDefinition(name = "Bad Name", type = "object"))),
  ...
  fun `validate flags itemSchema on non-sequential media types`() {
    val response =
        EndpointResponse(
            ...
            content =
                mapOf(
                    "application/json" to
                        MediaTypeObject(itemSchema = SchemaProperty(types = setOf("object")))))
    val endpoint =
        EndpointDefinition(
            path = "/events",
            method = HttpMethod.GET,
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Events", "1.0"), paths = mapOf("/events" to PathItem(get = endpoint)))
                                                        ...
                                                        mapOf(
                                                            "200" to
                                                                EndpointResponse(
                                                                    ...
                                                                    description = "ok"))))))),
            responses =
                mapOf("202" to EndpointResponse(statusCode = "202", description = "accepted")))
    val definition =
        OpenApiDefinition(
  ...
  fun `validate flags parameter and header content with multiple media types`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Multi", "1.0"), paths = mapOf("/multi" to PathItem(get = endpoint)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any { it.contains("Parameter content must contain exactly one media type") })
    println(messages)
  ...
  fun `validate allows itemSchema for sequential media types`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Stream", "1.0"), paths = mapOf("/stream" to PathItem(get = endpoint)))
  ...
  fun `validate flags positional encoding without array schema`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Parts", "1.0"), paths = mapOf("/parts" to PathItem(get = endpoint)))
            ...
            components =
                Components(
                    securitySchemes =
                        mapOf(
                            "oidc" to
                                SecurityScheme(type = "openIdConnect", openIdConnectUrl = "bad"),
  ...
  fun `validate warns when encoding headers include content type`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Upload", "1.0"), paths = mapOf("/upload" to PathItem(post = endpoint)))
  ...
  fun `validate flags parameter content with serialization fields`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Search", "1.0"), paths = mapOf("/search" to PathItem(get = endpoint)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any {
          it.contains("Parameters using content must not define style/explode/allowReserved")
        })
  }
  ...
  fun `validate flags encoding entries without matching schema properties`() {
                        ...
                        MediaTypeObject(
                            schema =
                                SchemaProperty(
                                    types = setOf("object"),
                                    properties =
                                        mapOf("file" to SchemaProperty(types = setOf("string")))),
                            encoding =
                                mapOf(
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Uploads", "1.0"), paths = mapOf("/upload" to PathItem(post = endpoint)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any { it.contains("Encoding entry 'missing' has no matching schema property") })
  }
    ...
    val extraParamOp =
        EndpointDefinition(
            ...
            parameters =
                listOf(
                    EndpointParameter(
                        name = "id", type = "String", location = ParameterLocation.PATH),
                    EndpointParameter(
                        name = "oops", type = "String", location = ParameterLocation.PATH)),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", description = "ok")))

    val definition =
        OpenApiDefinition(
            info = Info("Users", "1.0"),
            paths = mapOf("/users/{id}/{id}" to PathItem(get = missingParamOp, put = extraParamOp)))
  ...
  fun `validate flags templated path collisions`() {
    ...
    val definition =
        OpenApiDefinition(
            ...
            paths =
                mapOf(
                    "/pets/{petId}" to PathItem(get = opA), "/pets/{name}" to PathItem(get = opB)))
                            ...
                            mapOf(
                                "env" to
                                    domain.ServerVariable(
                                        default = "prod", enum = listOf("prod")))),
                    Server(url = "https://{region}.example.com")),
            paths =
                mapOf(
                    "/ping" to
                        PathItem(
            ...
            servers =
                listOf(
                    Server(
                        ...
                        variables = mapOf("env{bad}" to domain.ServerVariable(default = "prod")))),
            paths =
                mapOf(
                    "/ping" to
                        PathItem(
  ...
  fun `validate flags invalid additionalOperations method token`() {
    ...
    val definition =
        OpenApiDefinition(
            ...
            paths =
                mapOf("/copy" to PathItem(additionalOperations = mapOf("BAD METHOD" to invalidOp))))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any {
          it.contains("additionalOperations method 'BAD METHOD' must be a valid HTTP token")
        })
  }
    ...
    val response =
        EndpointResponse(
            ...
            headers =
                mapOf(
                    "Content-Type" to
                        Header(type = "String", schema = SchemaProperty(types = setOf("string"))),
                    "X-Multi" to
                        Header(
                            type = "String",
                            content =
  ...
  fun `validate flags duplicate path item parameters`() {
    ...
    val pathItem =
        PathItem(
            ...
            get = endpoint)
    val definition =
        OpenApiDefinition(info = Info("Items", "1.0"), paths = mapOf("/items" to pathItem))
        ...
        OpenApiDefinition(
            ...
            paths =
                mapOf(
                    "/shared" to
                        PathItem(
                            ref = "#/components/pathItems/Shared", summary = "Shared operations")))
  ...
  fun `validate flags link operation target rules`() {
        ...
        EndpointResponse(
            ...
            links =
                mapOf(
                    ...
                    "bothTargets" to
                        Link(operationId = "getUser", operationRef = "#/paths/~1users/get")))
    val endpoint =
        EndpointDefinition(
            path = "/users",
            method = HttpMethod.GET,
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Links", "1.0"), paths = mapOf("/users" to PathItem(get = endpoint)))
    ...
    println(messages)
    assertTrue(
        messages.any { it.contains("Link must not define both operationId and operationRef") })
  }
  ...
  fun `validate accepts operationRef with encoded path template braces`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Links", "1.0"), paths = mapOf("/users/{id}" to PathItem(get = endpoint)))
  ...
  fun `validate supports self based operationRef`() {
        ...
        EndpointResponse(
            ...
            links =
                mapOf(
                    ...
                    "bad" to
                        Link(operationRef = "https://example.com/openapi#/paths/~1missing/get")))
    val endpoint =
        EndpointDefinition(
            path = "/users",
            method = HttpMethod.GET,
    ...
    assertTrue(
        messages.any {
          it.contains("operationRef 'https://example.com/openapi#/paths/~1missing/get'")
        })
    assertTrue(
        messages.none {
          it.contains("operationRef 'https://example.com/openapi#/paths/~1users/get'")
        })
  }
  ...
  fun `validate resolves relative self operationRef using base uri`() {
    val response =
        EndpointResponse(
            ...
            links =
                mapOf(
                    "good" to
                        Link(operationRef = "https://example.com/api/openapi#/paths/~1users/get"),
                    "bad" to
                        Link(
                            operationRef = "https://example.com/api/openapi#/paths/~1missing/get")))
    val endpoint =
        EndpointDefinition(
            path = "/users",
            method = HttpMethod.GET,
    ...
    val messages =
        validator.validate(definition, baseUri = "https://example.com/root/openapi").map {
          it.message
        }
    println(messages)
    assertTrue(
        messages.any {
          it.contains("operationRef 'https://example.com/api/openapi#/paths/~1missing/get'")
        })
    assertTrue(
        messages.none {
          it.contains("operationRef 'https://example.com/api/openapi#/paths/~1users/get'")
        })
  }
  ...
  fun `validate flags undefined security requirement schemes`() {
    ...
    println(messages)
    assertTrue(
        messages.any { it.contains("Security requirement references undefined scheme 'Missing'") })
    println(messages)
    assertTrue(
        messages.any {
          it.contains("Security requirement references undefined scheme 'UnknownScheme'")
        })
  ...
  fun `validate flags encoding usage outside multipart and form urlencoded`() {
    ...
    val formMedia =
        MediaTypeObject(prefixEncoding = listOf(EncodingObject(contentType = "text/plain")))
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Encoding", "1.0"), paths = mapOf("/submit" to PathItem(post = endpoint)))
  ...
  fun `validate flags parameter style constraints and missing content`() {
    ...
    val queryStringParam =
        EndpointParameter(name = "raw", type = "String", location = ParameterLocation.QUERYSTRING)
    val missingSchemaParam =
        EndpointParameter(name = "missing", type = "String", location = ParameterLocation.QUERY)
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Params", "1.0"), paths = mapOf("/params" to PathItem(get = endpoint)))
  ...
  fun `validate flags unique operationIds and tags`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Ops", "1.0"),
            tags = listOf(domain.Tag("users"), domain.Tag("users")),
            paths = mapOf("/one" to PathItem(get = opA), "/two" to PathItem(post = opB)))
  ...
  fun `validate flags operationId duplicates across callbacks and component path items`() {
                ...
                mapOf(
                    "OnEvent" to
                        Callback.Inline(
                            expressions =
                                mapOf(
                                    "\$request.body#/url" to PathItem(post = callbackOperation)))))
    val componentOperation =
        EndpointDefinition(
            path = "/components",
            method = HttpMethod.GET,
            operationId = "dupCallback",
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Callbacks", "1.0"),
            components =
                Components(pathItems = mapOf("Shared" to PathItem(get = componentOperation))),
  ...
  fun `validate flags invalid response codes`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Codes", "1.0"), paths = mapOf("/codes" to PathItem(get = operation)))
  ...
  fun `validate flags server variable default outside enum`() {
    ...
    println(messages)
    assertTrue(
        messages.any { it.contains("Server variable default must be one of the enum values") })
  }
            ...
            servers =
                listOf(
                    Server(
                        ...
                        variables = mapOf("env" to domain.ServerVariable(default = "prod")))),
            paths =
                mapOf(
                    "/ping" to
                        PathItem(
  ...
  fun `validate flags media type and encoding exclusivity`() {
    ...
    assertTrue(
        messages.any {
          it.contains("Media type must not define encoding with prefixEncoding/itemEncoding")
        })
    println(messages)
    assertTrue(
        messages.any {
          it.contains("Encoding object must not define encoding with prefixEncoding/itemEncoding")
        })
  }
  ...
  fun `validate flags security scheme required fields and oauth flow requirements`() {
            ...
            "badFlow" to
                SecurityScheme(
                    ...
                    flows =
                        OAuthFlows(
                            authorizationCode =
                                OAuthFlow(authorizationUrl = "https://auth.example.com"))))
    ...
    println(messages)
    assertTrue(
        messages.any { it.contains("openIdConnect security scheme requires openIdConnectUrl") })
    println(messages)
    assertTrue(
        messages.any {
          it.contains("authorizationCode OAuth flow requires authorizationUrl and tokenUrl")
        })
  }
  ...
  fun `validate flags allowEmptyValue outside query parameters`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Empty", "1.0"), paths = mapOf("/empty" to PathItem(get = endpoint)))
  ...
  fun `validate warns on reserved header parameter names`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Reserved", "1.0"), paths = mapOf("/reserved" to PathItem(get = endpoint)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any {
          it.contains("Header parameters named Accept, Content-Type, or Authorization")
        })
  }
  ...
  fun `validate flags discriminator without composition`() {
    val schema =
        domain.SchemaDefinition(
            name = "Pet",
            type = "object",
            properties = mapOf("petType" to SchemaProperty(types = setOf("string"))),
            required = listOf("petType"),
            discriminator = Discriminator(propertyName = "petType"))
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"), components = Components(schemas = mapOf("Pet" to schema)))
  ...
  fun `validate flags discriminator missing defaultMapping when optional`() {
    val schema =
        domain.SchemaDefinition(
            ...
            type = "object",
            properties = mapOf("petType" to SchemaProperty(types = setOf("string"))),
            oneOfSchemas = listOf(SchemaProperty(ref = "#/components/schemas/Cat")),
            discriminator = Discriminator(propertyName = "petType"))
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"), components = Components(schemas = mapOf("Pet" to schema)))
  ...
  fun `validate flags invalid xml usage`() {
    val attrSchema =
        domain.SchemaDefinition(
            name = "Attr", type = "string", xml = Xml(nodeType = "attribute", attribute = true))
    val wrappedSchema =
        domain.SchemaDefinition(name = "Wrapped", type = "string", xml = Xml(wrapped = true))
    val definition =
        OpenApiDefinition(
            info = Info("Xml", "1.0"),
            components =
                Components(schemas = mapOf("Attr" to attrSchema, "Wrapped" to wrappedSchema)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any { it.contains("xml.attribute must not be set when xml.nodeType is present") })
    println(messages)
  ...
  fun `validate flags invalid link runtime expressions`() {
            ...
            links =
                mapOf(
                    "next" to
                        Link(
                            operationId = "getNext",
                            parameters = mapOf("id" to "\$response.body#/id", "bad" to "\$bad"))))
    val endpoint =
        EndpointDefinition(
            path = "/pets",
            method = HttpMethod.GET,
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Links", "1.0"), paths = mapOf("/pets" to PathItem(get = endpoint)))
  ...
  fun `validate warns when only non-success response is defined`() {
    val endpoint =
        EndpointDefinition(
            ...
            operationId = "failure",
            responses =
                mapOf("400" to EndpointResponse(statusCode = "400", description = "bad request")))
    val definition =
        OpenApiDefinition(
            info = Info("Failure", "1.0"), paths = mapOf("/failure" to PathItem(get = endpoint)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any {
          it.contains("Only one response is defined and it is not a success response")
        })
  }
  ...
  fun `validate flags invalid reference uri`() {
    ...
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"), paths = mapOf("/pets" to PathItem(post = endpoint)))
  ...
  fun `validate skips requestBody and header rules when references are present`() {
    val endpoint =
        EndpointDefinition(
            ...
            operationId = "createPet",
            requestBody =
                RequestBody(reference = ReferenceObject("#/components/requestBodies/PetCreate")),
            responses =
                mapOf(
                    "200" to
                        EndpointResponse(
    ...
    val messages = validator.validate(definition).map { it.message }
    assertTrue(
        messages.none { it.contains("RequestBody content should contain at least one media type") })
  ...
  fun `validate flags invalid example externalValue uri`() {
    val definition =
        OpenApiDefinition(
            ...
            components =
                Components(
                    examples = mapOf("BadExample" to ExampleObject(externalValue = "not a uri"))))
                ...
                Components(
                    schemas =
                        mapOf(
                            "BadSchema" to
                                SchemaDefinition(
                                    name = "BadSchema", type = "object", ref = "not a uri"))))
  ...
  fun `validate resolves callback references with absolute uri`() {
        ...
        PathItem(
            post =
                EndpointDefinition(
                    ...
                    operationId = "onEvent",
                    responses =
                        mapOf("200" to EndpointResponse(statusCode = "200", description = "ok"))))
                ...
                mapOf(
                    "onEvent" to
                        Callback.Reference(
                            ReferenceObject(
                                ref =
                                    "https://example.com/openapi.yaml#/components/callbacks/OnEvent"))),
            responses = mapOf("200" to EndpointResponse(statusCode = "200", description = "ok")))
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"),
    ...
    val messages = issues.map { it.message }
    assertTrue(
        messages.none { it.contains("Callback reference") && it.contains("could not be resolved") })
  }

  @Test
  fun `validate flags invalid schema bounds and ordering`() {
    val badLengths =
        SchemaDefinition(name = "BadLengths", type = "string", minLength = -1, maxLength = 2)
    val badObject =
        SchemaDefinition(name = "BadObject", type = "object", minProperties = 5, maxProperties = 2)
    val definition =
        OpenApiDefinition(
            info = Info("Bounds", "1.0"),
            components =
                Components(schemas = mapOf("BadLengths" to badLengths, "BadObject" to badObject)))
    ...
    println(messages)
    assertTrue(
        messages.any { it.contains("minProperties must be less than or equal to maxProperties") })
  }

  @Test
  fun `validate flags contains bounds without contains`() {
    val schema = SchemaDefinition(name = "MissingContains", type = "array", minContains = 1)
    val definition =
        OpenApiDefinition(
            info = Info("Contains", "1.0"),
            components = Components(schemas = mapOf("MissingContains" to schema)))

    val messages = validator.validate(definition).map { it.message }
    println(messages)
    assertTrue(
        messages.any {
          it.contains("minContains/maxContains are ignored without a contains schema")
        })
  }
  ...
  fun `validate flags invalid content metadata`() {
    ...
    val definition =
        OpenApiDefinition(
            ...
            components = Components(schemas = mapOf("ContentMeta" to schema)))
  ...
  fun `validate warns when oas keywords appear under json schema dialect`() {
    val schema = SchemaDefinition(name = "Pet", type = "object", xml = Xml(name = "pet"))
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"),
  ...
  fun `validate warns when custom keywords appear under json schema dialect`() {
    val schema =
        SchemaDefinition(name = "Pet", type = "object", customKeywords = mapOf("x-extra" to true))
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"),
            jsonSchemaDialect = "https://json-schema.org/draft/2020-12/schema",
  ...
  fun `validate errors when schema ref targets missing component`() {
    val schema = SchemaDefinition(name = "Pet", ref = "#/components/schemas/Missing")
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"), components = Components(schemas = mapOf("Pet" to schema)))
  ...
  fun `validate errors when schema ref targets missing defs entry`() {
    val schema =
        SchemaDefinition(
            name = "Pet",
            type = "object",
            defs = mapOf("Known" to SchemaProperty(types = setOf("string"))),
            properties = mapOf("missing" to SchemaProperty(ref = "#/\$defs/Unknown")))
    val definition =
        OpenApiDefinition(
            info = Info("Pets", "1.0"), components = Components(schemas = mapOf("Pet" to schema)))
  ...
  fun `validate errors when response link key is invalid`() {
    val response =
        EndpointResponse(
            statusCode = "200",
            description = "ok",
            links = mapOf("bad key" to Link(operationId = "listPets")))
    val endpoint =
        EndpointDefinition(
            path = "/pets",
            method = HttpMethod.GET,
