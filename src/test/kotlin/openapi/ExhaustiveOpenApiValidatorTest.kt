      ...
      val prop =
          SchemaProperty(
              ...
              discriminator =
                  Discriminator(
                      propertyName = "type",
                      mapping = if (i % 2 == 0) emptyMap() else mapOf("type" to "map")),
              xml =
                  Xml(
                      nodeType = if (i % 2 == 0) "attribute" else null,
                      attribute = true,
                      wrapped = true),
                  ...
                  Components(
                      schemas =
                          mapOf(
                              "TestSchema" to
                                  SchemaDefinition(
                                      name = "TestSchema", properties = mapOf("prop" to prop)))))
              ...
              responses =
                  if (i % 2 == 0) emptyMap()
                      ...
                      mapOf(
                          ...
                          "999" to
                              EndpointResponse(
                                  "999", description = "Invalid code", type = "String")),
              parameters =
                  listOf(
                      EndpointParameter(
                          name = "param${i}",
                          type = "string",
                          location =
                              if (i % 2 == 0) ParameterLocation.QUERY else ParameterLocation.PATH,
                          ...
                          schema =
                              if (i % 2 == 0) SchemaProperty(types = setOf("string")) else null,
                          content =
                              if (i % 2 != 0) mapOf("application/json" to MediaTypeObject())
                              else emptyMap())),
              security =
                  if (i % 2 == 0) listOf(mapOf("invalidScheme" to emptyList())) else emptyList(),
              servers =
                  listOf(
                      Server(
                          url = "http://api.example.com/{var}?q=1",
                          variables = mapOf("var" to ServerVariable(default = "v")))))
      val def =
          OpenApiDefinition(
              info = Info("Test", "1.0"), paths = mapOf("/pets" to PathItem(get = op)))
  ...
  fun `test server variables validation permutations exhaustively`() {
    for (i in 1..50) {
      val server =
          Server(
              url =
                  if (i % 2 == 0) "http://example.com/{test}?query=1#frag"
                  else "http://example.com/{test}/{test}",
              variables =
                  if (i % 3 == 0) null
                  else
                      mapOf(
                          "test" to
                              ServerVariable(
                                  default = "val",
                                  enum = if (i % 4 == 0) emptyList() else listOf("val", "other")),
          ...
          OpenApiDefinition(
              info =
                  Info(
                      "Test",
                      "1.0",
                      termsOfService = "invalid-url",
                      contact = Contact(url = "invalid-url", email = "invalid-email")),
