...
/**
 * Shared utilities for mapping OpenAPI types to Kotlin types and vice versa. Updated to support OAS
 * 3.2:
...
object TypeMappers {
  ...
  /**
   * Maps an OpenAPI Schema Property to a Kotlin type string. Handles basics: string, integer,
   * number, boolean, array. Handles Dual Types (["string", "null"]) as Nullable Kotlin types.
   * Handles Binary Data (base64 encoded strings) as ByteArray. Resolves references via
   * [ReferenceResolver].
   */
  ...
  /**
   * Maps an OpenAPI Schema Property to a Kotlin type string, with optional dynamic reference
   * resolution support.
   *
  ...
  private fun mapTypeInternal(
    ...
    try {
      ...
      val primaryType =
          effectiveTypes.firstOrNull { it != "null" } ?: effectiveTypes.firstOrNull() ?: "string"
  ...
  private fun inferTypes(prop: SchemaProperty): Set<String> {
    if (prop.items != null ||
        prop.prefixItems.isNotEmpty() ||
        prop.contains != null ||
        prop.minItems != null ||
        prop.maxItems != null ||
        prop.uniqueItems != null) {
      return setOf("array")
    }

    if (prop.properties.isNotEmpty() ||
        prop.additionalProperties != null ||
        prop.patternProperties.isNotEmpty() ||
        prop.propertyNames != null ||
        prop.dependentSchemas.isNotEmpty() ||
        prop.minProperties != null ||
        prop.maxProperties != null ||
  ...
  /**
   * Maps a Kotlin type string (PSI representation) back to an OpenAPI Schema Property. e.g.
   * "List<String>?" -> types: ["array", "null"], items: { types: ["string"] } e.g. "ByteArray" ->
   * types: ["string"], contentEncoding: "base64" (OAS 3.1 style)
   */
