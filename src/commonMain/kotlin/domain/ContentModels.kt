...
data class ExampleObject(
    /**
     * Reference to an Example Object. When present, this is treated as a Reference Object and other
     * fields are ignored.
     */
    val ref: String? = null,
    ...
    /**
     * An example of the serialized form of the value, including encoding and escaping. If present,
     * `value` and `externalValue` MUST be absent.
     */
    val serializedValue: String? = null,
    /**
     * A URI that identifies the serialized example in a separate document. If present,
...
/**
 * A single encoding definition applied to a single value, with the mapping determined by the Media
 * Type Object.
 *
...
data class EncodingObject(
    /**
     * The Content-Type for encoding a specific property. The value is a comma-separated list of
     * media types.
     */
    val contentType: String? = null,
    /**
     * A map allowing additional information to be provided as headers. `Content-Type` is described
     * separately and SHALL be ignored in this section.
     */
    val headers: Map<String, Header> = emptyMap(),
    /**
     * Describes how a specific property value will be serialized depending on its type. Uses the
     * same values as [ParameterStyle].
     */
    val style: ParameterStyle? = null,
    /**
     * When true, property values of type `array` or `object` generate separate parameters for each
...
/**
 * Each Media Type Object describes content structured in accordance with the media type identified
 * by its key.
 *
...
data class MediaTypeObject(
    /**
     * Reference to a Media Type Object. When present, this is treated as a Reference Object and
...
data class RequestBody(
     ...
     *
     * This allows preserving the distinction between an omitted `content` field and an explicit
     * empty map (`content: {}`) for round-trip fidelity.
     */
 ...
 *
 * A callback can be expressed inline as a map of runtime expressions to [PathItem] objects, or as a
 * Reference Object that points to a reusable callback definition.
 *
...
sealed interface Callback {
  ...
  /**
   * Inline callback definition: runtime expression -> [PathItem]. Extensions are allowed as
   * siblings of the expression keys.
   */
  data class Inline(
      val expressions: Map<String, PathItem> = emptyMap(),
      val extensions: Map<String, Any?> = emptyMap(),
      /**
       * Reference metadata when this inline callback was resolved from a `$ref`. When present,
       * serializers SHOULD emit the `$ref` and ignore [expressions] for round-trip fidelity.
       */
      val reference: ReferenceObject? = null
  ) : Callback

  /** Reference to a reusable callback definition. */
  data class Reference(val reference: ReferenceObject) : Callback
}
