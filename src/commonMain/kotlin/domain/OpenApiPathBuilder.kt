 ...
 *
 * This is the inverse of [OpenApiPathFlattener] and is intended for Kotlin -> OpenAPI assembly
 * workflows.
 */
object OpenApiPathBuilder {
  ...
  fun buildPaths(
    ...
    return byPath.mapValues { (_, ops) -> buildPathItem(ops, liftCommonPathMetadata) }
  }
  ...
  private fun buildPathItem(
    ...
    val commonSummary =
        if (liftCommonPathMetadata) findCommonString(ops.map { it.summary }) else null
    val commonDescription =
        if (liftCommonPathMetadata) findCommonString(ops.map { it.description }) else null
    ...
    ops.forEach { ep ->
      val normalized =
          if (!liftCommonPathMetadata) {
            ep
          } else {
            val filteredParams =
                if (commonParams.isEmpty()) ep.parameters
                else ep.parameters.filterNot { commonParams.contains(it) }
