...
data class OpenApiDefinition(
     ...
     * OpenAPI document uses. The `openapi` field SHOULD be used by tooling to interpret the OpenAPI
     * document. This is *not* related to the `info.version` string, which describes the OpenAPI
     * document's version.
     */
     ...
     * `servers` field is not provided, or is an empty array, the default value would be a Server
     ...
     *
     * This maps the path string (e.g. "/users/{id}") to a [PathItem], which can contain multiple
     * operations (GET, POST, etc.) and path-level parameters.
     */
    val paths: Map<String, PathItem> = emptyMap(),

    /**
     * Specification extensions applied to the Paths Object (keys starting with `x-`). These are
     * siblings of path entries under the `paths` object.
     */
    val pathsExtensions: Map<String, Any?> = emptyMap(),

    /**
     * When true, serialize an explicit empty Paths Object (`paths: {}`). This preserves ACL-style
     * "no paths available" semantics distinct from omission.
     */
    ...
    /**
     * Specification extensions applied to the Webhooks Object (keys starting with `x-`). These are
     * siblings of webhook entries under the `webhooks` object.
     */
    val webhooksExtensions: Map<String, Any?> = emptyMap(),

    /**
     * When true, serialize an explicit empty Webhooks Object (`webhooks: {}`). This preserves
     * ACL-style "no webhooks available" semantics distinct from omission.
     */
    ...
    /**
     * When true, serialize an explicit empty security array (`security: []`). This distinguishes
     * between "not specified" and "explicitly no security requirements".
     */
    ...
    /**
     * The self-assigned URI of this document, which also serves as its base URI. This maps to the
     * `$self` field in the OpenAPI Object. This MUST be in the form of a URI reference.
     */
...
data class Info(
     ...
     * Specification version or the version of the API being described or the version of the OpenAPI
     * Description).
     */
    val version: String,
...
data class License(
    ...
    /**
     * A URI for the license used for the API. This MUST be in the form of a URI. The `url` field is
...
data class Server(
     ...
     * relative, to indicate that the host location is relative to the location where the document
     * containing the Server Object is being served.
     */
    val url: String,
...
data class SecurityScheme(
    ...
    /**
     * **REQUIRED** (`apiKey`). The location of the API key. Valid values are `"query"`, `"header"`,
     * or `"cookie"`.
     */
    val `in`: String? = null,

    /**
     * **REQUIRED** (`http`). The name of the HTTP Authentication scheme to be used in the
     * Authorization header. e.g. "bearer", "basic", "digest".
     */
    val scheme: String? = null,

    /**
     * (`http`, "bearer"). A hint to the client to identify how the bearer token is formatted. e.g.
     * "JWT".
     */
    val bearerFormat: String? = null,

    /**
     * **REQUIRED** (`oauth2`). An object containing configuration information for the flow types
     * supported.
     */
    val flows: OAuthFlows? = null,

    /**
     * **REQUIRED** (`openIdConnect`). Well-known URL to discover the OpenID Connect provider
     * metadata.
     */
    ...
    /**
     * Declares this security scheme to be deprecated. Consumers SHOULD refrain from usage of the
     * declared scheme. Default value is `false`.
     */
...
data class OAuthFlow(
    /**
     * **REQUIRED** (`implicit`, `authorizationCode`). The authorization URL to be used for this
     * flow.
     */
    val authorizationUrl: String? = null,
 ...
 * Object.
 *
 * Properties are mapped as: Scheme Name -> List<Scope Name> An empty list of scopes means the
 * scheme alone is sufficient.
 *
 * See
 * [Security Requirement Object](https://spec.openapis.org/oas/v3.2.0#security-requirement-object)
 */
...
data class Tag(
     ...
     * description, and circular references between parent and child tags MUST NOT be used.
     */
    val parent: String? = null,

    /**
     * A machine-readable string to categorize what sort of tag it is. Any string value can be used;
     * common uses are `nav` for Navigation, `badge` for visible badges, `audience` for APIs used by
     * different groups.
     */
