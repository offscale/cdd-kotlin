...
/**
 * Supported HTTP Methods for API operations. Updated to include QUERY (OAS 3.2) support.
 *
...
enum class HttpMethod {
  ...
  QUERY,
  /**
   * Represents a non-standard or extension HTTP method. Use [EndpointDefinition.customMethod] to
   * provide the raw method token (e.g. "COPY").
   */
  CUSTOM
}

/**
 * The location of the parameter. Possible values are "query", "querystring", "header", "path" or
 * "cookie".
 *
...
/**
 * Serialization style for parameters. Describes how the parameter value will be serialized
 ...
 * and location. Note: For `in: querystring`, the parameter represents the *entire* query string and
...
data class EndpointParameter(
    /**
     * **REQUIRED**. The name of the parameter. Parameter names are case-sensitive. If `in` is
     * "path", the name field MUST correspond to a single template expression occurring within the
     ...
     * value is REQUIRED and its value MUST be true. Otherwise, the field MAY be included and its
     * default value is false.
     */
    val isRequired: Boolean = true,

    /**
     * The schema defining the type used for the parameter. Mutually exclusive with [content] in the
     * OpenAPI spec.
     */
    val schema: SchemaProperty? = null,

    /**
     * A map containing the representations for the parameter. Mutually exclusive with [schema] in
     * the OpenAPI spec.
     */
    ...
    /**
     * When true, clients MAY pass a zero-length string value in place of parameters that would
     * otherwise be omitted entirely.
     */
    ...
    /**
     * Example of the parameter's potential value. Equivalent to the Parameter Object's `example`
     * field (shorthand Example Object).
     */
    val example: ExampleObject? = null,

    /**
     * Examples of the parameter's potential value. Equivalent to the Parameter Object's `examples`
     * field.
     */
    val examples: Map<String, ExampleObject> = emptyMap(),

    /**
     * Reference Object allowing `$ref` with optional summary/description overrides. When present,
     * this is treated as a Reference Object and other fields are ignored for serialization.
     */
 ...
 * representations. The Header Object follows the structure of the Parameter Object with the
 * following changes:
...
data class Header(
    ...
    /**
     * The schema defining the type used for the header. Mutually exclusive with [content] in the
     * OpenAPI spec.
     */
    val schema: SchemaProperty? = null,

    /**
     * A map containing the representations for the header. Mutually exclusive with [schema] in the
     * OpenAPI spec.
     */
    val content: Map<String, MediaTypeObject> = emptyMap(),

    /**
     * A brief description of the header. CommonMark syntax MAY be used for rich text
     * representation.
     */
    val description: String? = null,
    ...
    /**
     * Specifies that the header is deprecated and SHOULD be transitioned out of usage. Default
     * value is `false`.
     */
    val deprecated: Boolean = false,
    ...
    /**
     * When this is true, header values of type `array` or `object` generate a single header whose
     * value is a comma-separated list of the array items or key-value pairs of the map. The default
     * value is `false`.
     */
...
data class EndpointResponse(
     ...
     *
     * This preserves the distinction between an omitted `content` field and an explicit empty map
     * (`content: {}`) for round-trip fidelity.
     */
    ...
    /**
     * The Kotlin class name of the response payload, or null if no content is returned (Unit). This
     * abstracts the `content` map for simplified code generation. Legacy-friendly for Swagger 2.0 /
     * OAS 3.0 style generators.
     */
...
data class Link(
    /**
     * Reference to a Link Object. When present, this is treated as a Reference Object and other
     * fields are ignored.
     */
    val ref: String? = null,
...
data class EndpointDefinition(
    /**
     * The value of the Path Item Key (e.g. "/users/{id}"). Not strictly inside the Operation
     * Object, but required for context.
     */
    val path: String,
    ...
    /**
     * Raw method token for [HttpMethod.CUSTOM] (e.g. "COPY", "PROPFIND"). Ignored for standard
     * methods.
     */
    ...
    /**
     * Whether `operationId` was explicitly present in the OpenAPI document. When false, serializers
     * SHOULD omit `operationId` to preserve spec-legal omission.
     */
    ...
    /**
     * The Kotlin class name of the request body, or null if none. Abstracts `requestBody.content`.
     ...
     * identifier for the Callback Object. Each value is either an inline map of Runtime Expressions
     * to Path Items or a Reference Object to a reusable callback definition.
     *
     * Example: `{"onDataProcessed": Callback.Inline(expressions = {"{$request.query.callbackUrl}":
     * PathItem(...)})}`
     *
     ...
     * includes alternative Security Requirement Objects that can be used.
     */
    ...
    /**
     * An alternative `servers` array to service this operation. If specified, it overrides any
     * servers defined at the Path Item or OpenAPI root.
     */
  ...
  /**
   * Returns the raw HTTP method token for this operation. For standard methods, this is the enum
   * name (e.g. "GET"). For custom methods, this is [customMethod] or "CUSTOM".
   */
  val methodName: String
    get() = if (method == HttpMethod.CUSTOM) customMethod ?: "CUSTOM" else method.name

  /**
   * Helper to get the primary success type (lowest 2xx code). Used for return type generation in
   * Ktor clients.
   */
...
data class PathItem(
    ...
    /**
     * A map of additional operations on this path. The key is the HTTP method with the same
     * capitalization that is to be sent in the request. Must not contain methods covered by fixed
     * fields (e.g. no `POST` entry).
     */
