 ...
 *
 * This parser is intentionally lenient: missing required fields are defaulted where possible to
 * keep round-trip workflows resilient.
 */
class OpenApiParser() {
  ...
  private inline fun <T> withSelfBase(self: String?, baseUri: String?, block: () -> T): T {
   ...
   *
   * If the root contains an `openapi` field, it is treated as an OpenAPI Object. Otherwise, the
   * root is parsed as a Schema Object (boolean or object).
   */
  ...
  private fun parseOpenApi(
    ...
    return withSelfBase(self, baseUri) {
      ...
      val components =
          if (isSwagger2) parseSwagger2Components(root) else parseComponents(root.get("components"))
  ...
  private fun parsePathItem(path: String, node: JsonNode, components: Components?): PathItem {
    ...
    val additionalOpsFromField =
        parseAdditionalOperations(obj.get("additionalOperations"), path, components)
    ...
    return PathItem(
        ...
        delete =
            parseOperationIfPresent(
                path, "delete", HttpMethod.DELETE, obj.get("delete"), components),
        options =
            parseOperationIfPresent(
                path, "options", HttpMethod.OPTIONS, obj.get("options"), components),
        head = parseOperationIfPresent(path, "head", HttpMethod.HEAD, obj.get("head"), components),
        patch =
            parseOperationIfPresent(path, "patch", HttpMethod.PATCH, obj.get("patch"), components),
        trace =
            parseOperationIfPresent(path, "trace", HttpMethod.TRACE, obj.get("trace"), components),
        query =
            parseOperationIfPresent(path, "query", HttpMethod.QUERY, obj.get("query"), components),
  ...
  private fun parseOperation(
    ...
    val requestBodyType =
        selectSchema(requestBody?.content, components)?.let { TypeMappers.mapType(it) }
  ...
  private fun parseCallbackObject(node: JsonNode, components: Components?): Callback {
    ...
    if (ref != null) {
      ...
      val resolved =
          components?.let { resolveCallbackComponentRef(Callback.Reference(reference), it) }
    ...
    if (ref != null) {
      ...
      if (resolved != null) {
        return resolved.copy(
            description = reference.description ?: resolved.description, reference = reference)
      }
  ...
  private fun parseRequestBody(node: JsonNode?, components: Components?): RequestBody? {
    ...
    if (ref != null) {
      ...
      return resolved?.copy(
          description = reference.description ?: resolved.description, reference = reference)
          ?: RequestBody(description = reference.description, reference = reference)
    }
  ...
  private fun parseResponse(
    ...
    return EndpointResponse(
        ...
        schema = schema,
        examples =
            obj?.get("examples")?.asObject()?.fields()?.asSequence()?.associate { (k, v) ->
  ...
  private fun parseHeader(node: JsonNode, components: Components?): Header {
    ...
    if (ref != null) {
      ...
      return resolved?.copy(
          description = reference.description ?: resolved.description, reference = reference)
          ?: Header(type = "String", description = reference.description, reference = reference)
    }
  ...
  private fun parseLinks(node: JsonNode?, components: Components?): Map<String, Link> {
    val obj = node.asObject() ?: return emptyMap()
    return obj.fields().asSequence().associate { (name, raw) -> name to parseLink(raw, components) }
  }
  ...
  private fun parseLink(node: JsonNode, components: Components?): Link {
    ...
    if (ref != null) {
      ...
      return resolved?.copy(
          ...
          reference = reference,
          description = reference.description ?: resolved.description)
          ?: Link(ref = ref, reference = reference, description = obj.text("description"))
    }
    return Link(
  ...
  private fun parseMediaTypeObject(node: JsonNode, components: Components?): MediaTypeObject {
    ...
    if (ref != null) {
      ...
      return resolved?.copy(ref = ref, reference = reference)
          ?: MediaTypeObject(ref = ref, reference = reference)
    }
  ...
  private fun parseEncodingObject(node: JsonNode, components: Components?): domain.EncodingObject {
  ...
  private fun resolveComponentRefs(components: Components): Components {
    val resolvedParameters =
        components.parameters.mapValues { (_, param) ->
          resolveParameterComponentRef(param, components)
        }
    val resolvedResponses =
        components.responses.mapValues { (_, response) ->
          resolveResponseComponentRef(response, components)
        }
    val resolvedRequestBodies =
        components.requestBodies.mapValues { (_, body) ->
          resolveRequestBodyComponentRef(body, components)
        }
    val resolvedHeaders =
        components.headers.mapValues { (_, header) ->
          resolveHeaderComponentRef(header, components)
        }
    val resolvedLinks =
        components.links.mapValues { (_, link) -> resolveLinkComponentRef(link, components) }
    val resolvedExamples =
        components.examples.mapValues { (_, example) ->
          resolveExampleComponentRef(example, components)
        }
    val resolvedMediaTypes =
        components.mediaTypes.mapValues { (_, media) ->
          resolveMediaTypeComponentRef(media, components)
        }
    val resolvedSecuritySchemes =
        components.securitySchemes.mapValues { (_, scheme) ->
          resolveSecuritySchemeComponentRef(scheme, components)
        }
    val resolvedCallbacks =
  ...
  private fun resolveParameterComponentRef(
    ...
    return resolved.copy(
        description = mergedDescription, reference = reference, extensions = mergedExtensions)
  }
  ...
  private fun resolveResponseComponentRef(
    ...
    val lookup =
        resolveComponentLookup(ref, "responses", components, contextBase) ?: return response
  ...
  private fun resolveRequestBodyComponentRef(
    ...
    val lookup =
        resolveComponentLookup(ref, "requestBodies", components, contextBase) ?: return body
  ...
  private fun resolveMediaTypeComponentRef(
    ...
    return resolved.copy(ref = ref, reference = reference, extensions = mergedExtensions)
  }
  ...
  private fun resolveCallbackComponentRef(
    ...
    return when (callback) {
      ...
      is Callback.Reference -> {
        ...
        val lookup =
            resolveComponentLookup(ref, "callbacks", components, contextBase) ?: return callback
  ...
  private fun resolveSecuritySchemeComponentRef(
    ...
    val lookup =
        resolveComponentLookup(ref, "securitySchemes", components, contextBase) ?: return scheme
    ...
    val resolved =
        resolveSecuritySchemeComponentRef(target, lookup.components, visited, lookup.base)
  ...
  private fun parseSchemaDefinition(name: String, node: JsonNode): SchemaDefinition {
    if (node.isBoolean) {
      return SchemaDefinition(
          name = name, booleanSchema = node.booleanValue(), type = "object", typeExplicit = false)
    }
    val obj = node.asObject()
  ...
  private fun parseSchemaProperties(node: JsonNode?): Map<String, SchemaProperty> {
    val obj = node.asObject() ?: return emptyMap()
    return obj.fields().asSequence().associate { (name, raw) -> name to parseSchemaProperty(raw) }
  }
  ...
  private fun parseSchemaProperty(node: JsonNode): SchemaProperty {
    ...
    val legacyNullable = isLegacyNullable(obj)
    val inferredType =
        if (rawTypes.isEmpty() && legacyNullable) resolveSchemaType(rawTypes, obj) else null
  ...
  private fun parseSecuritySchemes(node: JsonNode?): Map<String, SecurityScheme> {
    val obj = node.asObject() ?: return emptyMap()
    return obj.fields().asSequence().associate { (name, raw) -> name to parseSecurityScheme(raw) }
  }
  ...
  private fun parseSecurityScheme(node: JsonNode): SecurityScheme {
    ...
    if (ref != null) {
      val reference =
          ReferenceObject(
              ref = ref, summary = obj.text("summary"), description = obj.text("description"))
      return SecurityScheme(reference = reference, description = reference.description)
    }
    return SecurityScheme(
        ...
        scopes =
            obj.get("scopes")?.asObject()?.fields()?.asSequence()?.associate { (k, v) ->
              k to (v.textValue() ?: v.toString())
            },
        deprecated = obj.boolean("deprecated") ?: false,
        extensions = parseExtensions(obj))
  ...
  private fun defaultOperationId(methodToken: String, path: String): String {
    ...
    return raw.replace(Regex("[^a-zA-Z0-9_]"), "_").replace(Regex("_+"), "_").trim('_').ifEmpty {
      methodToken.lowercase()
    }
  }
  ...
  private fun resolveSchemaType(types: Set<String>, obj: JsonNode?): String {
    ...
    inferEnumType(obj.get("enum"))?.let {
      return it
    }
    ...
    if (obj.has("properties") ||
        ...
        obj.has("patternProperties") ||
        obj.has("propertyNames") ||
        obj.has("dependentSchemas") ||
        obj.has("required") ||
        obj.has("minProperties") ||
        obj.has("maxProperties")) {
      return "object"
    }
    ...
    if (obj.has("minimum") ||
        obj.has("maximum") ||
        obj.has("multipleOf") ||
        obj.has("exclusiveMinimum") ||
        obj.has("exclusiveMaximum")) {
      return "number"
    }
    ...
    if (obj.has("minLength") ||
        obj.has("maxLength") ||
        obj.has("pattern") ||
        obj.has("contentEncoding") ||
        obj.has("contentMediaType") ||
        obj.has("format")) {
      return "string"
    }
  ...
  private fun parseSchemaRefs(node: JsonNode?): List<String> {
    val array = node.asArray() ?: return emptyList()
    return array.mapNotNull { item -> item.text("\$ref") ?: item.takeIf { it.isTextual }?.asText() }
  }
  ...
  private fun parseComponentHeaders(node: JsonNode?): Map<String, Header> {
    val obj = node.asObject() ?: return emptyMap()
    return obj.fields().asSequence().associate { (name, raw) -> name to parseHeader(raw, null) }
  }
...
private fun JsonNode?.asArray(): List<JsonNode>? =
    if (this != null && this.isArray) this.elements() else null

private fun JsonNode.text(field: String): String? =
    this.get(field)?.takeIf { !it.isNullNode }?.asText()
