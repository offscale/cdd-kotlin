 ...
 *
 * This provides the Kotlin -> OpenAPI bridge by assembling schemas into components and endpoints
 * into paths.
 */
class OpenApiAssembler {
   ...
   * @param pathItems Optional Path Item metadata to merge into built paths
   *   (summary/description/parameters/servers/extensions).
   * @param liftCommonPathMetadata When true, lift shared operation metadata
   *   (params/summary/description/servers) to the Path Item for more concise OpenAPI output.
   */
  fun assemble(
      info: Info,
  ...
  private fun mergeComponents(base: Components?, schemas: List<SchemaDefinition>): Components? {
          ...
          when {
            ...
            existing == null -> meta
                ...
                existing.copy(
                    ...
                    parameters =
                        if (meta.parameters.isNotEmpty()) meta.parameters else existing.parameters,
