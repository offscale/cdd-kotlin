...
/**
 * Parses Kotlin source code to extract API Endpoint definitions implementation details. Analyzes
 * Ktor `client.request` calls to reverse-engineer the API Spec, including querystring parameters
 * set via `url.encodedQuery`, plus KDoc metadata such as callbacks, response summaries,
 * operation-level extensions, and interface-level OpenAPI metadata.
 */
...
class NetworkParser {
  ...
  private fun parseFunction(func: KtNamedFunction): EndpointDefinition? {
    ...
    val finalParams =
        parameters.map { param ->
          val funcArg =
              func.valueParameters.find {
                it.name == param.name || "`" + it.name + "`" == param.name
              }
          ...
          val deprecated =
              funcArg?.annotationEntries?.any { it.shortName?.asString() == "Deprecated" } ?: false
          val isOptional =
              funcArg?.let {
          ...
          RequestBody(
              content =
                  mapOf(
                      mediaType to
                          MediaTypeObject(
                              schema = TypeMappers.kotlinToSchemaProperty(requestBodyType))),
  ...
  private fun extractPathParams(path: String): List<String> {
    val regex = "\\{([^}]+)}".toRegex()
    return regex.findAll(path).map { it.groupValues[1] }.filter { it != "baseUrl" }.toList()
  }
  ...
  private fun inferParamNameFromLoop(expression: KtCallExpression): String? {
    val forEachCall =
        expression.parents.filterIsInstance<KtCallExpression>().firstOrNull {
          it.calleeExpression?.text == "forEach"
        } ?: return null
    val dotQualified = forEachCall.parent
    val receiver =
        dotQualified
            ?.let { it as? org.jetbrains.kotlin.psi.KtDotQualifiedExpression }
            ?.receiverExpression
            ?.text
  ...
  private fun extractParamExamples(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, ParamExampleBundle> {
    val docComment = element.docComment ?: return emptyMap()
  ...
  private fun extractParamMeta(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, ParamMeta> {
    val docComment = element.docComment ?: return emptyMap()
    ...
    lines
        .filter { it.startsWith("@paramStyle") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@paramExplode") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@paramAllowReserved") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@paramAllowEmptyValue") }
        .forEach { line ->
  ...
  private fun extractParamSchemas(
      element: org.jetbrains.kotlin.psi.KtDeclaration
    ...
    lines
        .filter { it.startsWith("@paramSchema") }
        .forEach { line ->
  ...
  private fun extractParamContents(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, Map<String, MediaTypeObject>> {
    val docComment = element.docComment ?: return emptyMap()
    val lines = docComment.text.split("\n").map { cleanKDocLine(it) }
    val contents = mutableMapOf<String, Map<String, MediaTypeObject>>()
    lines
        .filter { it.startsWith("@paramContent") }
        .forEach { line ->
  ...
  private fun extractParamRefs(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, ReferenceObject> {
    val docComment = element.docComment ?: return emptyMap()
    val lines = docComment.text.split("\n").map { cleanKDocLine(it) }
    val refs = mutableMapOf<String, ReferenceObject>()
    lines
        .filter { it.startsWith("@paramRef") }
        .forEach { line ->
  ...
  private fun extractParamExtensions(
      element: org.jetbrains.kotlin.psi.KtDeclaration
    ...
    lines
        .filter { it.startsWith("@paramExtensions") }
        .forEach { line ->
  ...
  private fun extractResponseRefs(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, ReferenceObject> {
    val docComment = element.docComment ?: return emptyMap()
    val lines = docComment.text.split("\n").map { cleanKDocLine(it) }
    val refs = mutableMapOf<String, ReferenceObject>()
    lines
        .filter { it.startsWith("@responseRef") }
        .forEach { line ->
  ...
  private fun extractResponseExtensions(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, Map<String, Any?>> {
    val docComment = element.docComment ?: return emptyMap()
    val lines = docComment.text.split("\n").map { cleanKDocLine(it) }
    val extensions = mutableMapOf<String, Map<String, Any?>>()
    lines
        .filter { it.startsWith("@responseExtensions") }
        .forEach { line ->
  ...
  private fun extractExternalDocs(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ...
  private fun extractTags(element: org.jetbrains.kotlin.psi.KtDeclaration): List<String> {
    ...
    val tagLine =
        docComment.text.split("\n").map { cleanKDocLine(it) }.find { it.startsWith("@tag") }
            ?: return emptyList()
  ...
  private fun extractServers(element: org.jetbrains.kotlin.psi.KtDeclaration): List<Server> {
    val docComment = element.docComment ?: return emptyList()
    val serverLine =
        docComment.text.split("\n").map { cleanKDocLine(it) }.find { it.startsWith("@servers") }
            ?: return emptyList()
  ...
  private fun extractOpenApiLine(lines: List<String>): OpenApiLine {
    val line =
        lines.firstOrNull { it.startsWith("@openapi") } ?: return OpenApiLine(null, null, null)
    val json = line.removePrefix("@openapi").trim()
    if (json.isEmpty()) return OpenApiLine(null, null, null)
    val node =
        runCatching { jsonMapper.readTree(json) }.getOrNull()
            ?: return OpenApiLine(null, null, null)
  ...
  private fun extractSecurityLines(lines: List<String>): SecurityParseResult {
    ...
    val explicitEmpty =
        lines.any { it == "@securityEmpty" || it.startsWith("@securityEmpty ") } &&
            securityLines.isEmpty()

    val requirements =
        securityLines.mapNotNull { line ->
  ...
  private fun extractExtensionsLine(lines: List<String>): Map<String, Any?> {
    ...
    extensionLines.forEach { line ->
      ...
      val node = runCatching { jsonMapper.readTree(json) }.getOrNull() ?: return@forEach
      if (!node.isObject) return@forEach
      node.fields().forEachRemaining { (key, value) -> merged[key] = nodeToValue(value) }
    }
    return merged
  }
  ...
  private fun extractPathItemsLine(lines: List<String>): Map<String, domain.PathItem> {
    ...
    return openApiParser
        .parseString(jsonMapper.writeValueAsString(wrapper), OpenApiParser.Format.JSON)
        .paths
  }
  ...
  private fun extractSecuritySchemesLine(lines: List<String>): Map<String, SecurityScheme> {
    ...
    val parsed =
        openApiParser.parseString(jsonMapper.writeValueAsString(root), OpenApiParser.Format.JSON)
  ...
  private fun extractRequestBody(
    ...
    val docComment = element.docComment ?: return null
    val line =
        docComment.text.split("\n").map { cleanKDocLine(it) }.find { it.startsWith("@requestBody") }
            ?: return null
  ...
  private fun mergeRequestBody(
    ...
    val mergedRequired =
        if (overrideResult.requiredPresent) overrideBody.required else baseBody.required
    val mergedDescription = overrideBody.description ?: baseBody.description
    val mergedExtensions =
        if (overrideBody.extensions.isNotEmpty()) overrideBody.extensions else baseBody.extensions
  ...
  private fun extractResponses(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, EndpointResponse> {
    val docComment = element.docComment ?: return emptyMap()
    val responses = mutableMapOf<String, EndpointResponse>()
    val lines = docComment.text.split("\n").map { cleanKDocLine(it) }
    lines
        .filter { it.startsWith("@response ") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@responseHeaders") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@responseLinks") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@responseContent") }
        .forEach { line ->
    ...
    lines
        .filter { it.startsWith("@responseSummary") }
        .forEach { line ->
    ...
    extractResponseRefs(element).forEach { (code, reference) ->
      ...
      responses[code] =
          existing.copy(summary = summary, description = description, reference = reference)
    }
  ...
  private data class RequestBodyParseResult(val body: RequestBody, val requiredPresent: Boolean)

  private fun extractSecurity(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): SecurityParseResult {
  ...
  private fun parseReferenceObjectValue(raw: String): ReferenceObject? {
    ...
    if (node != null) {
      ...
      return ReferenceObject(
          ref = ref, summary = obj.text("summary"), description = obj.text("description"))
    }
  ...
  private fun parseHeadersJson(json: String): Map<String, Header> {
    val node = runCatching { jsonMapper.readTree(json) }.getOrNull() ?: return emptyMap()
    if (!node.isObject) return emptyMap()
    return node.fields().asSequence().associate { (name, raw) -> name to parseHeaderNode(raw) }
  }

  private fun parseLinksJson(json: String): Map<String, Link> {
    val node = runCatching { jsonMapper.readTree(json) }.getOrNull() ?: return emptyMap()
    if (!node.isObject) return emptyMap()
    return node.fields().asSequence().associate { (name, raw) -> name to parseLinkNode(raw) }
  }
  ...
  private fun parseHeaderNode(node: JsonNode): Header {
    ...
    if (ref != null) {
      val reference =
          ReferenceObject(
              ref = ref, summary = obj.text("summary"), description = obj.text("description"))
      return Header(type = "String", description = reference.description, reference = reference)
    }
  ...
  private fun parseLinkNode(node: JsonNode): Link {
    ...
    if (ref != null) {
      val reference =
          ReferenceObject(
              ref = ref, summary = obj.text("summary"), description = obj.text("description"))
      return Link(ref = ref, reference = reference, description = obj.text("description"))
    }
    return Link(
        operationId = obj.text("operationId"),
        operationRef = obj.text("operationRef"),
        parameters =
            obj.get("parameters")?.asObject()?.fields()?.asSequence()?.associate { (k, v) ->
              k to nodeToValue(v)
            } ?: emptyMap(),
  ...
  private fun parseContentNode(node: JsonNode?): Map<String, MediaTypeObject> {
    ...
    return obj.fields().asSequence().associate { (name, raw) -> name to parseMediaTypeNode(raw) }
  }
  ...
  private fun parseEncodingMapNode(node: JsonNode?): Map<String, EncodingObject> {
    val obj = node.asObject() ?: return emptyMap()
    return obj.fields().asSequence().associate { (name, raw) -> name to parseEncodingNode(raw) }
  }
  ...
  private fun parseHeadersNode(node: JsonNode?): Map<String, Header> {
    val obj = node.asObject() ?: return emptyMap()
    return obj.fields().asSequence().associate { (name, raw) -> name to parseHeaderNode(raw) }
  }
  ...
  private fun parseSchemaPropertyNode(node: JsonNode): SchemaProperty {
    ...
    return SchemaProperty(
        ...
        unevaluatedProperties =
            obj?.get("unevaluatedProperties")?.let { parseSchemaPropertyNode(it) },
  ...
  private fun parseDiscriminatorNode(node: JsonNode?): Discriminator? {
    ...
    return Discriminator(
        ...
        mapping =
            obj.get("mapping")?.asObject()?.fields()?.asSequence()?.associate { (k, v) ->
  ...
  private fun parseExternalDocsNode(node: JsonNode?): ExternalDocumentation? {
    ...
    return ExternalDocumentation(
        description = obj.text("description"), url = url, extensions = parseExtensions(obj))
  }
  ...
  private fun extractDescription(element: org.jetbrains.kotlin.psi.KtDeclaration): String? {
    val docComment = element.docComment ?: return null
    val descLine =
        docComment.text.split("\n").map { cleanKDocLine(it) }.find { it.startsWith("@description") }
            ?: return null
  ...
  private fun extractDeprecated(element: org.jetbrains.kotlin.psi.KtDeclaration): Boolean {
    ...
    val hasDocTag =
        docComment
            ...
            ?.split("\n")
            ?.map { cleanKDocLine(it) }
            ?.any { it == "@deprecated" || it.startsWith("@deprecated ") } ?: false
    val hasAnnotation =
        (element as? org.jetbrains.kotlin.psi.KtModifierListOwner)?.annotationEntries?.any {
  ...
  private fun parseParameterStyle(value: String): domain.ParameterStyle? {
    return when (value.lowercase()) {
      ...
      "spacedelimited",
      "space_delimited",
      "spaceDelimited" -> domain.ParameterStyle.SPACE_DELIMITED
      "pipedelimited",
      "pipe_delimited",
      "pipeDelimited" -> domain.ParameterStyle.PIPE_DELIMITED
      "deepobject",
      "deep_object",
      "deepObject" -> domain.ParameterStyle.DEEP_OBJECT
      else -> null
    }
  }
  ...
  private fun cleanKDocLine(line: String): String {
    return line.trim().replace("/**", "").replace("*/", "").replaceFirst("*", "").trim()
  }
}

private fun JsonNode?.asObject(): JsonNode? = if (this != null && this.isObject) this else null

private fun JsonNode?.asArray(): List<JsonNode>? =
    if (this != null && this.isArray) this.toList() else null
...
private fun JsonNode.boolean(field: String): Boolean? =
    this.get(field)?.takeIf { it.isBoolean }?.booleanValue()

private fun JsonNode.int(field: String): Int? = this.get(field)?.takeIf { it.isNumber }?.intValue()

private fun JsonNode.double(field: String): Double? =
    this.get(field)?.takeIf { it.isNumber }?.doubleValue()
