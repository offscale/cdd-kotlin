...
/**
 * Generates Ktor Network Interface and Implementation code from Endpoint Definitions. Supports:
 * - Result<T> return types
 * - Path, Query, Querystring, Header, Cookie parameters
 * - Parameter Serialization Styles (Matrix, Label, Form, Pipe/Space Delimited, DeepObject) and
 *   Explode logic
 ...
 * - Interface-level KDoc for root OpenAPI metadata (info, servers, security, tags, externalDocs,
 *   extensions, securitySchemes)
...
class NetworkGenerator {
  ...
  fun generateApi(
    ...
    val resolvedServers =
        if (resolvedMetadata.servers.isNotEmpty()) resolvedMetadata.servers else servers
    val resolvedSecuritySchemes =
        if (resolvedMetadata.securitySchemes.isNotEmpty()) {
          resolvedMetadata.securitySchemes
        } else {
          securitySchemes
        }
    val resolvedWebhooks =
        if (resolvedMetadata.webhooks.isEmpty() && webhooks.isEmpty()) {
          emptyMap()
        } else {
    ...
    val needsQueryStringEncoding =
        endpoints.any { ep ->
          ep.parameters.any {
            it.location == ParameterLocation.QUERYSTRING && it.content.isNotEmpty()
          }
        }
    val needsParameterContentSerialization =
        endpoints.any { ep ->
          ep.parameters.any {
            it.location != ParameterLocation.QUERYSTRING && it.content.isNotEmpty()
          }
        }
    val needsFormBodyEncoding =
        endpoints.any { ep ->
          resolveRequestContentType(ep)?.let { isFormUrlEncodedMediaType(it) } == true
        }
    val needsMultipartBodyEncoding =
        endpoints.any { ep ->
          resolveRequestContentType(ep)?.let { isMultipartFormDataMediaType(it) } == true
        }
    val needsMultipartPositionalEncoding =
        endpoints.any { ep -> requiresMultipartPositionalEncoding(ep) }
    ...
    val needsOAuthHelpers =
        resolvedSecuritySchemes.values.any { scheme ->
    ...
    val companionObject =
        buildCompanionObject(resolvedServers, resolvedSecuritySchemes, serverSupport)

    val baseUrlParam =
        when {
          serverSupport != null -> {
            val parts = mutableListOf("serverIndex: Int = 0", "serverName: String? = null")
          ...
          defaultUrl.isNotEmpty() ->
              "private val baseUrl: String = \"${escapeKotlinString(defaultUrl)}\""
          else -> "private val baseUrl: String = \"\""
        }

    val interfaceDoc =
        generateInterfaceKDoc(
            resolvedMetadata, resolvedWebhooks, resolvedServers, resolvedSecuritySchemes)
  ...
  private fun generateInterfaceKDoc(
    ...
    val hasOpenapi =
        metadata.openapi != null || metadata.jsonSchemaDialect != null || metadata.self != null
    ...
    if (!hasOpenapi &&
        ...
        !hasWebhooks &&
        !hasPathsExtensions &&
        !hasPathsExplicitEmpty &&
        !hasPathItems &&
        !hasWebhooksExtensions &&
        !hasWebhooksExplicitEmpty &&
        !hasComponentExamples &&
        !hasComponentLinks &&
        !hasComponentCallbacks &&
        !hasComponentParameters &&
        !hasComponentResponses &&
        !hasComponentRequestBodies &&
        !hasComponentHeaders &&
        !hasComponentPathItems &&
        !hasComponentMediaTypes &&
    ...
    if (hasComponentParameters) {
      sb.append(
          " * @componentParameters ${renderComponentParameters(metadata.componentParameters)}\n")
    }
    ...
    if (hasComponentRequestBodies) {
      sb.append(
          " * @componentRequestBodies ${renderComponentRequestBodies(metadata.componentRequestBodies)}\n")
    }
    ...
    if (hasComponentMediaTypes) {
      sb.append(
          " * @componentMediaTypes ${renderComponentMediaTypes(metadata.componentMediaTypes)}\n")
    }
  ...
  private fun renderPathItems(pathItems: Map<String, PathItem>): String {
    ...
    val tempDefinition =
        OpenApiDefinition(info = Info(title = "PathItems", version = "0.0.0"), paths = pathItems)
  ...
  private fun renderWebhooks(webhooks: Map<String, PathItem>): String {
    ...
    val tempDefinition =
        OpenApiDefinition(info = Info(title = "Webhooks", version = "0.0.0"), webhooks = webhooks)
    val json = openApiWriter.writeJson(tempDefinition)
    val node = jsonMapper.readTree(json)
  ...
  private fun buildHelperFunctions(endpoints: List<EndpointDefinition>): String {
    ...
    val needsSequentialJsonEncoding = endpoints.any { ep -> requiresSequentialJsonRequest(ep) }
    val needsSequentialJsonDecoding = endpoints.any { ep -> requiresSequentialJsonResponse(ep) }
    val needsPathEncoding =
        endpoints.any { ep -> ep.parameters.any { it.location == ParameterLocation.PATH } }
    val needsAllowReserved =
        endpoints.any { ep ->
  ...
  private data class ServerSupport(val hasVariables: Boolean, val companionSnippet: String)

  private data class QueryParamLines(val lines: List<String>, val emptyValueLine: String?)
  ...
  private fun buildServerSupport(servers: List<Server>): ServerSupport? {
    ...
    val snippet =
        listOf(serverSpecBlock, variablesBlock, helpersBlock)
            .filter { it.isNotBlank() }
            .joinToString("\n\n")

    return ServerSupport(hasVariables = hasVariables, companionSnippet = snippet)
  }
    ...
    val entries =
        servers.map { server ->
          ...
          if (variables.isNotEmpty()) {
            val vars =
                variables.entries.joinToString(", ") { (name, variable) ->
  ...
  private fun buildServerVariablesBlock(servers: List<Server>): String {
    ...
    allVariables.entries.forEachIndexed { index, (rawName, variable) ->
      ...
      if (enumValues.isNotEmpty()) {
        ...
        val defaultConst =
            constants.firstOrNull { it.second == variable.default }?.first
  ...
  private fun buildServerHelperFunctions(hasVariables: Boolean): String {
    val variablesParam =
        if (hasVariables) ", variables: ServerVariables = ServerVariables()" else ""
    val variablesMap = if (hasVariables) "variables.toMap()" else "emptyMap()"
    val serverSpecFallback =
        if (hasVariables) {
          "ServerSpec(url = \"/\", name = null, variables = emptyMap())"
        } else {
          "ServerSpec(url = \"/\", name = null)"
        }
    val defaultBaseUrlSignature =
        "fun defaultBaseUrl(serverIndex: Int = 0, serverName: String? = null$variablesParam): String"
  ...
  private fun toPascalCase(raw: String): String {
    ...
    val joined =
        parts.joinToString("") { part -> part.lowercase().replaceFirstChar { it.uppercase() } }
  ...
  private fun resolveServerUrlTemplate(template: String, variables: Map<String, String>): String {
  ...
  private fun generateAuthFactory(schemes: Map<String, SecurityScheme>): String {
    ...
    if (hasMutualTls && mutualTlsConfigurerName != null) {
      ...
      schemes
          .filter { it.value.type == "mutualTLS" }
          .forEach { (key, _) ->
            val paramName = schemeParamNames[key] ?: sanitizeIdentifier(key)
            installBlocks.append(
                """
  ...
  private fun generateKDoc(ep: EndpointDefinition): String {
    ...
    val paramsWithMeta =
        ep.parameters.filter {
          it.style != null ||
              it.explode != null ||
              it.allowReserved != null ||
              it.allowEmptyValue != null
        }
    ...
    if (hasParamMeta) {
      paramsWithMeta.forEach { param ->
        param.style?.let {
          sb.append("     * @paramStyle ${param.name} ${parameterStyleValue(it)}\n")
        }
  ...
  private fun mediaTypeToDocValue(mediaType: MediaTypeObject): Any {
    mediaType.reference?.let {
      return referenceToDocValue(it)
    }
    mediaType.ref?.let {
      return mapOf("\$ref" to it)
    }
    val map = linkedMapOf<String, Any?>()
  ...
  private fun schemaPropertyToDocValue(schema: SchemaProperty): Any {
    ...
    if (schema.patternProperties.isNotEmpty()) {
      map["patternProperties"] =
          schema.patternProperties.mapValues { schemaPropertyToDocValue(it.value) }
    }
    ...
    if (schema.dependentSchemas.isNotEmpty()) {
      map["dependentSchemas"] =
          schema.dependentSchemas.mapValues { schemaPropertyToDocValue(it.value) }
    }
    ...
    schema.unevaluatedProperties?.let {
      map["unevaluatedProperties"] = schemaPropertyToDocValue(it)
    }
  ...
  fun generateMethodImpl(ep: EndpointDefinition): String {
    ...
    val queryStringParam =
        ep.parameters.firstOrNull { it.location == ParameterLocation.QUERYSTRING }
    if (queryStringParam != null && queryParams.isNotEmpty()) {
      throw IllegalArgumentException(
          "OAS 3.2: querystring and query parameters cannot be used together for ${ep.operationId}")
    }
    ...
    val pathTemplate =
        ep.parameters
            ...
            .fold(ep.path) { currentPath, param ->
              ...
              val isMatrixOrLabel =
                  param.style == ParameterStyle.MATRIX || param.style == ParameterStyle.LABEL

              // If Matrix or Label style, we absorb the preceding slash if present
              val target =
                  if (isMatrixOrLabel && currentPath.contains("/$placeholder")) {
                    "/$placeholder"
                  } else {
                    placeholder
                  }
    ...
    val fullUrl =
        if (pathTemplate.startsWith("/")) "$baseUrlExpr$pathTemplate"
        else "$baseUrlExpr/$pathTemplate"

    // 2. Build Query/Header/Cookie Config
    val paramLines =
        ep.parameters.flatMap { param ->
          val optional = isOptionalParam(param)
          ...
          when (param.location) {
            ...
            ParameterLocation.FORMDATA -> emptyList<String>()
          }
        }
    ...
    if (queryStringParam != null && queryStringContentType == null) {
      val cleanType =
          resolveParameterType(queryStringParam, isOptionalParam(queryStringParam)).replace(" ", "")
      val isString = cleanType == "String" || cleanType == "String?"
      if (!isString) {
        throw IllegalArgumentException(
            "OAS 3.2: querystring parameter must be String when content is not defined for ${ep.operationId}")
      }
    }

    val queryStringConfig =
        if (queryStringParam != null) {
          val assignment =
              if (queryStringContentType != null) {
    ...
    val sequentialRequestElementType =
        if (contentType != null &&
            bodySignature != null &&
            isSequentialJsonMediaType(contentType)) {
    ...
    val prefixEncodingContentTypesLiteral =
        renderPrefixEncodingContentTypesLiteral(requestMediaType?.value)
    val prefixEncodingHeadersLiteral = renderPrefixEncodingHeadersLiteral(requestMediaType?.value)
    val itemEncodingContentTypeLiteral =
        renderItemEncodingContentTypeLiteral(requestMediaType?.value)
    ...
    val bodyConfig =
        if (bodySignature != null) {
          ...
          if (contentType != null &&
              !isFormUrlEncoded &&
              !isMultipartFormData &&
              isConcreteMediaType(contentType)) {
          ...
          val encodingAllowReservedArg =
              encodingAllowReservedLiteral?.let { ", allowReserved = $it" } ?: ""
          val encodingHeadersArg = encodingHeadersLiteral?.let { ", headers = $it" } ?: ""
          val positionalArgsList = mutableListOf<String>()
          prefixEncodingContentTypesLiteral?.let {
            positionalArgsList.add("prefixContentTypes = $it")
          }
          ...
          val positionalArgs =
              if (positionalArgsList.isNotEmpty()) {
          ...
          val bodyLine =
              when {
                ...
                usesPositionalMultipart -> "setBody(encodeMultipartPositional(body$positionalArgs))"
                isFormUrlEncoded ->
                    "setBody(encodeFormBody(body$encodingArg$encodingStyleArg$encodingExplodeArg$encodingAllowReservedArg))"
                isMultipartFormData ->
                    "setBody(encodeMultipartBody(body$encodingArg$encodingHeadersArg))"
                else -> "setBody(body)"
              }
          lines.add(bodyLine)
    ...
    val sequentialResponseElementType =
        if (responseContentType != null && isSequentialJsonMediaType(responseContentType)) {
          extractListElementType(returnType)
        } else {
          null
        }
    val successBodyExpr =
        if (sequentialResponseElementType != null) {
  ...
  private fun resolveResponseMediaTypeEntry(
    ...
    val success = ep.responses.keys.filter { it.startsWith("2") }.minOrNull() ?: return null
  ...
  private fun resolveResponseType(ep: EndpointDefinition): String {
    ...
    val success = ep.responses.keys.filter { it.startsWith("2") }.minOrNull() ?: return "Unit"
  ...
  private fun selectSchema(content: Map<String, MediaTypeObject>): SchemaProperty? {
    ...
    preferred.schema?.let {
      return it
    }
    preferred.itemSchema?.let {
      return wrapItemSchemaAsArray(it)
    }
  ...
  private fun selectPreferredMediaTypeEntry(
    ...
    val scored = entries.map { entry -> entry to mediaTypeScore(entry.key) }
    val comparator =
        Comparator<Pair<Map.Entry<String, MediaTypeObject>, MediaTypeScore>> { a, b ->
          val left = a.second
          val right = b.second
          when {
            left.specificity != right.specificity -> left.specificity.compareTo(right.specificity)
            left.jsonPreference != right.jsonPreference ->
                left.jsonPreference.compareTo(right.jsonPreference)
  ...
  private fun renderEncodingContentTypeLiteral(mediaType: MediaTypeObject?): String? {
    ...
    val entries =
        mediaType.encoding.mapNotNull { (name, encoding) ->
          encoding.contentType?.let { name to it }
        }
    if (entries.isEmpty()) return null
    val joined =
        entries.joinToString(", ") { (name, type) ->
  ...
  private fun renderEncodingStyleLiteral(mediaType: MediaTypeObject?): String? {
    ...
    if (entries.isEmpty()) return null
    val joined =
        entries.joinToString(", ") { (name, style) ->
  ...
  private fun renderEncodingExplodeLiteral(mediaType: MediaTypeObject?): String? {
    if (mediaType == null || mediaType.encoding.isEmpty()) return null
    val entries =
        mediaType.encoding.mapNotNull { (name, encoding) -> encoding.explode?.let { name to it } }
    if (entries.isEmpty()) return null
    val joined =
        entries.joinToString(", ") { (name, explode) ->
  ...
  private fun renderEncodingAllowReservedLiteral(mediaType: MediaTypeObject?): String? {
    ...
    if (entries.isEmpty()) return null
    val joined =
        entries.joinToString(", ") { (name, allowReserved) ->
  ...
  private fun renderEncodingHeadersLiteral(mediaType: MediaTypeObject?): String? {
    ...
    if (entries.isEmpty()) return null
    val joined =
        entries.joinToString(", ") { (name, headers) ->
          val headerLiteral =
              headers.entries.joinToString(", ") { (headerName, value) ->
  ...
  private fun renderPrefixEncodingContentTypesLiteral(mediaType: MediaTypeObject?): String? {
    ...
    val joined =
        mediaType.prefixEncoding.joinToString(", ") { encoding ->
  ...
  private fun renderPrefixEncodingHeadersLiteral(mediaType: MediaTypeObject?): String? {
    if (mediaType == null || mediaType.prefixEncoding.isEmpty()) return null
    val entries =
        mediaType.prefixEncoding.map { encoding ->
          val headerValues = extractEncodingHeaderValues(encoding)
          if (headerValues.isEmpty()) {
            "emptyMap()"
          } else {
            val headerLiteral =
                headerValues.entries.joinToString(", ") { (name, value) ->
  ...
  private fun extractEncodingHeaderValues(encoding: EncodingObject): Map<String, String> {
    ...
    encoding.headers.forEach { (headerName, header) ->
      ...
      val value =
          extractHeaderExampleValue(header) ?: extractHeaderDefaultValue(header) ?: return@forEach
  ...
  private fun extractHeaderExampleValue(header: Header): String? {
    header.example?.let { example ->
      extractExampleString(example)?.let {
        return it
      }
    }
  ...
  private fun buildQueryParamLines(param: EndpointParameter, paramType: String): QueryParamLines {
    if (param.content.isNotEmpty()) {
      if (param.style != null || param.explode != null || param.allowReserved != null) {
        throw IllegalArgumentException(
            "OAS 3.2: query parameters with content must not set style/explode/allowReserved for ${param.name}")
      }
  ...
  private fun buildHeaderParamLines(param: EndpointParameter, paramType: String): List<String> {
    if (param.content.isNotEmpty()) {
      if (param.style != null || param.explode != null || param.allowReserved != null) {
        throw IllegalArgumentException(
            "OAS 3.2: header parameters with content must not set style/explode/allowReserved for ${param.name}")
      }
      val contentType = resolveParameterContentType(param)
      val serialized =
          if (contentType != null) {
  ...
  private fun buildCookieParamLines(param: EndpointParameter, paramType: String): List<String> {
    if (param.content.isNotEmpty()) {
      if (param.style != null || param.explode != null || param.allowReserved != null) {
        throw IllegalArgumentException(
            "OAS 3.2: cookie parameters with content must not set style/explode/allowReserved for ${param.name}")
      }
      val contentType = resolveParameterContentType(param)
      val serialized =
          if (contentType != null) {
  ...
  private fun buildMatrixPathReplacement(
    ...
    return when {
      isList && explode ->
          ";$name=${template(buildPathArrayJoinExpr(name, ";$name=", allowReserved))}"
  ...
  private fun buildPathArrayJoinExpr(
      name: String,
      delimiter: String,
      allowReserved: Boolean
  ): String {
  ...
  private fun buildObjectJoinExpr(
      name: String,
