...
/**
 * Service that parses Kotlin source code to extract OpenAPI [SchemaDefinition]s. Analyzes Data
 * Classes (Objects), Enums, and Map typealiases. Supports:
...
class DtoParser {
  ...
  private fun parseTypeAlias(alias: KtTypeAlias): SchemaDefinition? {
    ...
    val isPrimitiveAlias =
        schemaProp.types.any { it in setOf("string", "integer", "number", "boolean") }
  ...
  private fun parseEnum(ktClass: KtClass): SchemaDefinition {
    ...
    val values =
        ktClass.declarations.filterIsInstance<org.jetbrains.kotlin.psi.KtEnumEntry>().mapNotNull {
            entry ->
          val serialNameEntry =
              entry.annotationEntries.find { it.shortName?.asString() == "SerialName" }
  ...
  private fun parseSealedInterface(
      ktClass: KtClass,
    ...
    val discriminatorAnnotation =
        ktClass.annotationEntries.find { it.shortName?.asString() == "JsonClassDiscriminator" }
    ...
    val inferredOneOf =
        if (oneOfTag.refs.isEmpty() && oneOfTag.schemas.isEmpty()) {
    ...
    ktClass.getProperties().forEach { prop ->
      ...
      val schemaProp =
          baseProp.copy(
              ...
              contentMediaType =
                  extractTagValue(prop, "contentMediaType") ?: baseProp.contentMediaType,
              contentEncoding =
                  extractTagValue(prop, "contentEncoding") ?: baseProp.contentEncoding,
  ...
  private fun parseDataClass(ktClass: KtClass): SchemaDefinition {
    ...
    parameters.forEach { param ->
      ...
      val serialNameEntry =
          param.annotationEntries.find { it.shortName?.asString() == "SerialName" }
      ...
      val schemaProp =
          baseProp.copy(
              ...
              contentMediaType =
                  extractTagValue(param, "contentMediaType") ?: baseProp.contentMediaType,
              contentEncoding =
                  extractTagValue(param, "contentEncoding") ?: baseProp.contentEncoding,
    ...
    val superTypes =
        ktClass.superTypeListEntries.map { it.text }.filter { it != "Any" && it != "Serializable" }
  ...
  private data class SealedSubtype(val kotlinName: String, val serialName: String?)
  ...
  private fun buildSealedSubtypeIndex(
    ...
    classes.forEach { candidate ->
      ...
      candidate.superTypeListEntries.forEach { entry ->
        ...
        index.getOrPut(simpleName) { mutableListOf() }.add(SealedSubtype(subtypeName, serialName))
      }
    }
  ...
  private fun extractSerialNameAnnotation(owner: KtModifierListOwner): String? {
    val serialNameEntry =
        owner.annotationEntries.find { it.shortName?.asString() == "SerialName" } ?: return null
    val arg = serialNameEntry.valueArgumentList?.arguments?.firstOrNull()
    val expression = arg?.getArgumentExpression() as? KtStringTemplateExpression
  ...
  private fun mergeDiscriminatorMappings(
    ...
    discriminator.mapping.forEach { (key, value) -> merged[key] = value }
  ...
  private fun extractExternalDocs(
    ...
    val seeLine =
        docComment.text.split("\n").map { cleanKDocLine(it) }.find { it.startsWith("@see") }
            ?: return null
  ...
  private fun extractDiscriminator(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): domain.Discriminator? {
    ...
    val mapping =
        (mappingLiteral as? Map<*, *>)?.entries?.associate { (k, v) ->
  ...
  private fun extractCustomKeywords(
      element: org.jetbrains.kotlin.psi.KtDeclaration
  ): Map<String, Any?> {
  ...
  private fun extractCompositionTag(
    ...
    val node =
        runCatching { jsonMapper.readTree(raw) }.getOrNull()
            ?: return CompositionTagResult(emptyList(), emptyList())
  ...
  private fun extractSchemaMapTag(
    ...
    return node.fields().asSequence().associate { (k, v) -> k to parseSchemaPropertyNode(v) }
  }
  ...
  private fun parseExternalDocsNode(node: JsonNode?): domain.ExternalDocumentation? {
    ...
    return domain.ExternalDocumentation(url = url, description = obj.text("description"))
  }
  ...
  private fun parseDocLiteral(raw: String): Any? {
    ...
    if (NUMBER_REGEX.matches(trimmed)) {
      return trimmed.toIntOrNull() ?: trimmed.toLongOrNull() ?: trimmed.toDoubleOrNull() ?: trimmed
    }
  ...
  private fun hasTag(element: org.jetbrains.kotlin.psi.KtDeclaration, tag: String): Boolean {
    ...
    val normalized = docComment.text.split("\n").map { cleanKDocLine(it) }
