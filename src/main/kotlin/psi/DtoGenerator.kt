...
/**
 * Generates Kotlin Data Transfer Objects (DTOs) from Schema Definitions. Uses kotlinx.serialization
 * annotations. Updated for OAS 3.2 Dual/Array Types support, additionalProperties maps, and OAS 3.1
 * Content Encoding.
 */
class DtoGenerator {
  ...
  /**
   * Generates a Kotlin file containing the Data Class, Enum, or Sealed Interface for the given
   * Schema.
   *
  ...
  private fun buildSource(packageName: String, definition: SchemaDefinition): String {
    ...
    if (definition.oneOf.isNotEmpty() ||
        definition.anyOf.isNotEmpty() ||
        definition.oneOfSchemas.isNotEmpty() ||
        definition.anyOfSchemas.isNotEmpty()) {
  ...
  private fun buildBooleanAliasSource(
      packageName: String,
      definition: SchemaDefinition,
      value: Boolean
  ): String {
  ...
  private fun buildRefAliasSource(
      packageName: String,
      definition: SchemaDefinition,
      ref: String
  ): String {
  ...
  private fun buildMapAliasSource(
    ...
    val valueType =
        TypeMappers.mapType(
            definition.additionalProperties!!, dynamicContext.scope::resolveDynamicRef)
  ...
  private fun buildSealedInterfaceSource(
    ...
    val propertyLines =
        definition.properties
            .map { (name, prop) ->
              val kotlinType =
                  resolveKotlinType(name, prop, definition.required, dynamicContext.scope)
              val desc = buildPropertyKDoc(prop)
              val deprecatedPropAnnotation =
                  buildDeprecatedAnnotation(prop.deprecated, indent = "    ")
  ...
  private fun buildEnumSource(packageName: String, definition: SchemaDefinition): String {
    val imports =
        mutableSetOf("kotlinx.serialization.Serializable", "kotlinx.serialization.SerialName")
    val kdoc = buildClassKDoc(definition)
    val deprecatedAnnotation = buildDeprecatedAnnotation(definition.deprecated)
  ...
  private fun buildClassKDoc(definition: SchemaDefinition): String {
    ...
    if (hasExamples) {
      definition.examples.forEach { (k, v) -> appendLine("@example $k: ${renderDocValue(v)}") }
    }
    if (hasEnum) {
      definition.enumValues!!.forEach { value -> appendLine("@enum ${renderEnumValue(value)}") }
    }
  ...
  private fun buildPropertyKDoc(prop: SchemaProperty): String {
    ...
    if (hasEnum) {
      prop.enumValues!!.forEach { value -> appendLine("@enum ${renderEnumValue(value)}") }
    }
  ...
  private fun renderDocValue(value: Any?): String {
    return when (value) {
      ...
      is Map<*, *>,
      is Iterable<*> -> jsonMapper.writeValueAsString(value)
      else -> value.toString()
    }
  }
  ...
  private fun renderSchemaComposition(refs: List<String>, inlines: List<SchemaProperty>): String {
  ...
  private fun schemaPropertyToDocValue(schema: SchemaProperty): Any {
    ...
    schema.dynamicRef?.let {
      return mapOf("\$dynamicRef" to it)
    }
