...
class OpenApiAssembler {
    ...
    private fun mergePathItems(
        ...
        overrides.forEach { (path, meta) ->
            ...
            val ref = meta.ref
            val useMeta = ref != null || existing == null
            if (useMeta) {
                merged[path] = meta
            } else {
                val sum = meta.summary ?: existing!!.summary
                val desc = meta.description ?: existing!!.description
                val params = if (meta.parameters.isNotEmpty()) meta.parameters else existing!!.parameters
                val srvs = if (meta.servers.isNotEmpty()) meta.servers else existing!!.servers
    ...
    private fun mergeExtensions(
        ...
        if (base.isEmpty()) { if (overrides.isEmpty()) return emptyMap() }
