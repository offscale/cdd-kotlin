...
class OpenApiParser(
    ...
    private fun parseLicense(node: JsonNode?): cdd.openapi.License? {
        val obj = node.asObject() ?: return null
        return cdd.openapi.License(
    ...
    private fun parseEncodingMap(
        node: JsonNode?,
        components: Components?
    ): Map<String, cdd.openapi.EncodingObject> {
        val obj = node.asObject() ?: return emptyMap()
        return obj.fields().asSequence().associate { (name, raw) ->
    ...
    private fun parseEncodingArray(
        node: JsonNode?,
        components: Components?
    ): List<cdd.openapi.EncodingObject> {
        val array = node.asArray() ?: return emptyList()
    ...
    private fun parseEncodingObject(
        node: JsonNode,
        components: Components?
    ): cdd.openapi.EncodingObject {
        val obj = node.asObject() ?: return cdd.openapi.EncodingObject()
        return cdd.openapi.EncodingObject(
