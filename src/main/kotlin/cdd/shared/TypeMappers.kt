...
object TypeMappers {
    ...
    private fun mapTypeInternal(
        ...
        if (stack.add(prop) == false) { return "Any" }
    ...
    private fun inferTypes(prop: SchemaProperty): Set<String> {
        ...
        prop.constValue?.let { value ->
            ...
            if (type.isNullOrBlank()) { return emptySet() }
            return setOf(type)
        }
    ...
    private fun inferTypesFromEnum(values: List<Any?>): Set<String> {
        ...
        if (types.size == 2) { if (types.contains("null")) return types }
        return emptySet()
    }

    /** Auto generated docs */ fun inferTypeFromValue(value: Any?): String? {
        return when (value) {
            null -> "null"
            is String -> "string"
