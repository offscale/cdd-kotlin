 ...
 *
 * This validator is intentionally focused on structural rules that are easy to enforce without full
 * semantic resolution or network access.
 *
 * Note: When a Reference Object is used for a Parameter, Header, RequestBody, Response, or Path
 * Item, validation short-circuits for that object to avoid false positives from
 * resolved/denormalized IR fields.
 */
class OpenApiValidator {
   ...
   *
   * @param baseUri Optional base URI used to resolve relative `$self` bases for link operationRef
   *   validation.
   */
  ...
  private fun validateSchemas(definition: OpenApiDefinition, issues: MutableList<OpenApiIssue>) {
    ...
    val state =
        SchemaValidationState(
            defaultDialect = defaultDialect, componentSchemaKeys = componentSchemaKeys)
  ...
  private fun validateSchemaDefinition(
    ...
    validateSchemaDefinition(
        schema, path, issues, state, dynamicContext, state.defaultDialect, emptySet())
  }
  ...
  private fun validateSchemaDefinition(
    ...
    schema.dynamicRef?.let { ref ->
      ...
      validateDynamicRefResolution(
          dynamicContext.root, ref, "$path.\$dynamicRef", issues, dynamicContext.scope)
    }
    ...
    schema.items?.let {
      validateSchemaPropertyInternal(
          it, "$path.items", issues, state, dynamicContext.scope, effectiveDialect, defsInScope)
    }
    ...
    schema.contains?.let {
      validateSchemaPropertyInternal(
          it, "$path.contains", issues, state, dynamicContext.scope, effectiveDialect, defsInScope)
    }
    ...
    schema.not?.let {
      validateSchemaPropertyInternal(
          it, "$path.not", issues, state, dynamicContext.scope, effectiveDialect, defsInScope)
    }
    schema.ifSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.if", issues, state, dynamicContext.scope, effectiveDialect, defsInScope)
    }
    schema.thenSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.then", issues, state, dynamicContext.scope, effectiveDialect, defsInScope)
    }
    schema.elseSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.else", issues, state, dynamicContext.scope, effectiveDialect, defsInScope)
    }
  }
  ...
  private fun validateSchemaProperty(
    ...
    val dynamicContext = buildDynamicAnchorContext(schema)
    validateSchemaPropertyInternal(
        schema, path, issues, state, dynamicContext.scope, state.defaultDialect, emptySet())
  }
  ...
  private fun validateSchemaPropertyInternal(
    ...
    validateDialectWarnings(schema, path, issues, effectiveDialect)
    val hasComposition =
        schema.oneOf.isNotEmpty() || schema.anyOf.isNotEmpty() || schema.allOf.isNotEmpty()
    validateDiscriminator(
        schema.discriminator, schema.required, schema.properties, hasComposition, path, issues)
    ...
    schema.defs.forEach { (name, sub) ->
      validateSchemaPropertyInternal(
          sub, "$path.\$defs.$name", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.items?.let {
      validateSchemaPropertyInternal(
          it, "$path.items", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.prefixItems.forEachIndexed { index, item ->
      validateSchemaPropertyInternal(
          item,
    ...
    schema.contains?.let {
      validateSchemaPropertyInternal(
          it, "$path.contains", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.properties.forEach { (name, prop) ->
      validateSchemaPropertyInternal(
          prop,
    ...
    schema.propertyNames?.let {
      validateSchemaPropertyInternal(
          it, "$path.propertyNames", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.additionalProperties?.let {
      validateSchemaPropertyInternal(
          it,
    ...
    schema.unevaluatedItems?.let {
      validateSchemaPropertyInternal(
          it, "$path.unevaluatedItems", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.contentSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.contentSchema", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.oneOf.forEachIndexed { index, prop ->
      validateSchemaPropertyInternal(
          prop, "$path.oneOf[$index]", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.anyOf.forEachIndexed { index, prop ->
      validateSchemaPropertyInternal(
          prop, "$path.anyOf[$index]", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.allOf.forEachIndexed { index, prop ->
      validateSchemaPropertyInternal(
          prop, "$path.allOf[$index]", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.not?.let {
      validateSchemaPropertyInternal(
          it, "$path.not", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.ifSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.if", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.thenSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.then", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
    schema.elseSchema?.let {
      validateSchemaPropertyInternal(
          it, "$path.else", issues, state, dynamicScope, effectiveDialect, defsInScope)
    }
  }
  ...
  private fun mergeDefsScope(current: Set<String>, defs: Set<String>): Set<String> {
    ...
    if (oasKeywords.isNotEmpty()) {
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "Schema dialect '$dialect' does not include the OpenAPI vocabulary; keywords " +
    ...
    if (hasCustomKeywords || hasExtensions) {
      ...
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "Schema dialect '$dialect' does not allow ${sources.joinToString(" and ")}; " +
    ...
    if (properties.isNotEmpty() && !properties.containsKey(disc.propertyName)) {
      issues +=
          OpenApiIssue(
              ...
              message =
                  "Discriminator property '${disc.propertyName}' is not defined in schema properties.")
    }
    if (!required.contains(disc.propertyName) && disc.defaultMapping.isNullOrBlank()) {
      issues +=
          OpenApiIssue(
              ...
              message =
                  "Discriminator defaultMapping is required when the property is not required.")
    }
  }
    ...
    if (!isSupportedOpenApiVersion(definition.openapi)) {
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "OpenAPI version '${definition.openapi}' is not 3.2.x; 3.2 features may be unsupported.")
    }
  }
  ...
  private fun validateServers(
    ...
    duplicateNames.forEach { (name, values) ->
      issues +=
          OpenApiIssue(
              ...
              path = basePath,
              message =
                  "Server name '$name' must be unique within the server list (found ${values.size}).")
    }
    servers.forEachIndexed { index, server ->
      ...
      val variablesInUrl =
          SERVER_VARIABLE_REGEX.findAll(server.url).map { it.groupValues[1] }.toList()
      ...
      duplicates.forEach { (name, values) ->
        issues +=
            OpenApiIssue(
                ...
                path = "$basePath[$index].url",
                message =
                    "Server variable '$name' must not appear more than once in the url (found ${values.size}).")
      }
      ...
      server.variables?.forEach { (name, variable) ->
        ...
        if (enumValues != null &&
            ...
            !enumValues.contains(variable.default)) {
          issues +=
              OpenApiIssue(
                  severity = OpenApiIssueSeverity.ERROR,
  ...
  private fun validatePaths(
    ...
    paths.forEach { (pathKey, item) ->
      ...
      validatePathItem(
          pathKey, item, "$basePath.$pathKey", issues, components, validateTemplating = true)
    }
  ...
  private fun validateWebhooks(
    ...
    webhooks.forEach { (name, item) ->
      validatePathItem(
          name, item, "$basePath.$name", issues, components, validateTemplating = false)
    }
  }
  ...
  private fun validatePathItem(
    ...
    if (item.ref != null && hasPathItemSiblings(item)) {
      issues +=
          OpenApiIssue(
              ...
              path = "$basePath.\$ref",
              message =
                  "Path Item with \$ref should not define other fields; sibling fields are ignored.")
    }
    if (item.ref != null) {
      return
    }
    ...
    item.delete?.let {
      validateOperation(it, "$basePath.delete", issues, components, item.parameters)
    }
    item.options?.let {
      validateOperation(it, "$basePath.options", issues, components, item.parameters)
    }
    item.head?.let { validateOperation(it, "$basePath.head", issues, components, item.parameters) }
    item.patch?.let {
      validateOperation(it, "$basePath.patch", issues, components, item.parameters)
    }
    item.trace?.let {
      validateOperation(it, "$basePath.trace", issues, components, item.parameters)
    }
    item.query?.let {
      validateOperation(it, "$basePath.query", issues, components, item.parameters)
    }
  ...
  private fun validateOperation(
      ...
      if (!isValidResponseCode(code)) {
        issues +=
            OpenApiIssue(
                ...
                message =
                    "Invalid response code '$code'. Use HTTP status codes, ranges (1XX-5XX), or 'default'.")
      }
    }
    ...
    val duplicates =
        operation.parameters.groupBy { it.name to it.location }.filter { it.value.size > 1 }
    duplicates.forEach { (key, values) ->
      issues +=
          OpenApiIssue(
              ...
              path = "$basePath.parameters",
              message =
                  "Duplicate parameter '${key.first}' in ${key.second} (count ${values.size}).")
    }
    ...
    operation.requestBody?.let {
      validateRequestBody(it, "$basePath.requestBody", issues, components)
    }
    ...
    if (param.location == ParameterLocation.HEADER &&
      ...
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "Header parameters named Accept, Content-Type, or Authorization are ignored by the spec.")
    }
      ...
      val allowed =
          when (param.location) {
            ParameterLocation.PATH ->
                setOf(ParameterStyle.MATRIX, ParameterStyle.LABEL, ParameterStyle.SIMPLE)
            ...
            ParameterLocation.FORMDATA -> emptySet()
          }
  ...
  private fun validatePathItemParameters(
    ...
    val duplicates = item.parameters.groupBy { it.name to it.location }.filter { it.value.size > 1 }
    duplicates.forEach { (key, values) ->
      issues +=
          OpenApiIssue(
              severity = OpenApiIssueSeverity.ERROR,
  ...
  private fun validateComponents(
    ...
    fun validateKeys(keys: Set<String>, componentPath: String) {
      keys
          .filterNot { componentKeyRegex.matches(it) }
          .forEach { key ->
            issues +=
                OpenApiIssue(
                    severity = OpenApiIssueSeverity.ERROR,
      ...
      if (looksLikeUriReference(name)) {
        issues +=
            OpenApiIssue(
                ...
                message =
                    "Security scheme component name '$name' looks like a URI; component names take precedence over URI references in Security Requirements.")
      }
    ...
    if (mediaTypeKey != null && !isValidMediaTypeKey(mediaTypeKey)) {
      issues +=
          OpenApiIssue(
              ...
              path = "$path",
              message =
                  "Invalid media type key '$mediaTypeKey'. Keys must be a media type or media type range.")
    }
      ...
      if (hasSiblings) {
        issues +=
            OpenApiIssue(
                ...
                path = path,
                message =
                    "Media type with \$ref should not define other fields (they are ignored).")
      }
      return
    }
    ...
    if (hasEncoding && !(isMultipart || isFormUrlEncoded)) {
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "Encoding is only applicable to multipart or application/x-www-form-urlencoded media types.")
    }
  ...
  private fun validateEncodingByName(
    ...
    media.encoding.keys
        .filterNot { properties.containsKey(it) }
        .forEach { key ->
          issues +=
              OpenApiIssue(
                  severity = OpenApiIssueSeverity.WARNING,
                  ...
                  message =
                      "Encoding entry '$key' has no matching schema property and will be ignored.")
        }
  }
    ...
    if (encoding.encoding.isNotEmpty() && hasPositional) {
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "Encoding object must not define encoding with prefixEncoding/itemEncoding.")
    }
    ...
    if (example.ref != null && (hasValue || hasDataValue || hasSerialized || hasExternal)) {
      issues +=
          OpenApiIssue(
              ...
              path = path,
              message =
                  "Example with \$ref must not define value/dataValue/serializedValue/externalValue.")
    }
    ...
    if (!validTypes.contains(scheme.type)) {
      issues +=
          OpenApiIssue(
              ...
              path = "$path.type",
              message =
                  "Security scheme type '${scheme.type}' is invalid. Must be one of: ${validTypes.joinToString()}.")
    }
    when (scheme.type) {
      "apiKey" -> {
      ...
      "openIdConnect" -> {
        if (scheme.openIdConnectUrl.isNullOrBlank()) {
          ...
          validateUrl(
              scheme.openIdConnectUrl, "$path.openIdConnectUrl", issues, requireHttps = true)
        }
      }
    }
  ...
  private fun validateOAuthFlows(
    ...
    flows.password?.let { flow ->
      ...
      flow.tokenUrl?.let { validateUrl(it, "$path.password.tokenUrl", issues, requireHttps = true) }
      flow.refreshUrl?.let {
      ...
      if (flow.deviceAuthorizationUrl.isNullOrBlank() || flow.tokenUrl.isNullOrBlank()) {
        issues +=
            OpenApiIssue(
                ...
                message =
                    "deviceAuthorization OAuth flow requires deviceAuthorizationUrl and tokenUrl.")
      }
      flow.deviceAuthorizationUrl?.let {
        validateUrl(
            it, "$path.deviceAuthorization.deviceAuthorizationUrl", issues, requireHttps = true)
      }
      flow.tokenUrl?.let {
  ...
  private fun validateOperationIds(
      definition: OpenApiDefinition,
      issues: MutableList<OpenApiIssue>
  ) {
  ...
  private fun validateLinkTargets(
        ...
        if (localRef != null && !operationRefs.contains(localRef)) {
          issues +=
              OpenApiIssue(
                  ...
                  path = path,
                  message =
                      "Link operationRef '$operationRef' does not resolve to a known operation.")
        }
      }
    }
    ...
    operations.forEach { op ->
      op.responses.forEach { (code, response) ->
        response.links?.forEach { (name, link) ->
          val basePath =
              "$.paths.${op.path}.${op.methodName.lowercase()}.responses.$code.links.$name"
  ...
  private fun validateUri(value: String, path: String, issues: MutableList<OpenApiIssue>) {
    validateUriInternal(
        value, path, issues, requireAbsolute = false, requireHttps = false, label = "URI")
  }
  ...
  private fun validateUrl(
    ...
    validateUriInternal(
        value, path, issues, requireAbsolute = true, requireHttps = requireHttps, label = "URL")
  }
  ...
  private fun extractEmbeddedRuntimeExpressions(raw: String): List<String> {
    ...
    return RUNTIME_EXPRESSION_EMBED_REGEX.findAll(raw)
        .mapNotNull { match ->
            ...
            if (expressions.isEmpty()) {
              issues +=
                  OpenApiIssue(
                      ...
                      path = "$path.$name",
                      message =
                          "Callback expression '$rawExpression' must include a valid runtime expression.")
              return@forEach
            }
            expressions.forEach { expression ->
              if (!isValidRuntimeExpression(expression)) {
                issues +=
                    OpenApiIssue(
                        ...
                        path = "$path.$name",
                        message =
                            "Callback expression '$rawExpression' contains invalid runtime expression '$expression'.")
              }
            }
          }
        }
      ...
      "\$statusCode" -> true
          ...
          when {
            ...
            value.startsWith("\$response.") ->
                isValidRuntimeSource(value.removePrefix("\$response."))
            else -> false
          }
    }
  }
  ...
  private fun isValidRuntimeSource(source: String): Boolean {
    return when {
      ...
      source.startsWith("body") -> {
        ...
        else if (!suffix.startsWith("#")) false else isValidJsonPointer(suffix.removePrefix("#"))
      }
      else -> false
    }
  }
  ...
  private data class ComponentRefInfo(val base: String?, val name: String)
  ...
  private fun validatePathTemplating(
    ...
    val templateParams = PATH_TEMPLATE_REGEX.findAll(pathKey).map { it.groupValues[1] }.toList()
    ...
    duplicates.forEach { (name, values) ->
      issues +=
          OpenApiIssue(
              ...
              path = basePath,
              message =
                  "Path template parameter '$name' must not appear more than once (found ${values.size}).")
    }
    ...
    operations.forEach { op ->
      val effectiveParams =
          (item.parameters + op.parameters).filter { it.location == ParameterLocation.PATH }
            ...
            if (!templateParams.contains(param.name)) {
              issues +=
                  OpenApiIssue(
                      ...
                      message =
                          "Path parameter '${param.name}' is not present in the path template.")
            }
          }
    }
  }
    ...
    callbacks.values.forEach { callback ->
      val resolved =
          when (callback) {
            ...
            is Callback.Reference ->
                resolveCallbackRef(callback.reference.ref, components) as? Callback.Inline
          }
  ...
  private fun collectAllOperations(definition: OpenApiDefinition): List<EndpointDefinition> {
    ...
    val rootOperations =
        collectOperations(definition.paths) +
            collectOperations(definition.webhooks) +
            collectOperations(components?.pathItems.orEmpty())
    val componentCallbackOps =
        collectCallbackOperations(components?.callbacks.orEmpty(), components)
  ...
  private fun encodeJsonPointerSegment(value: String): String {
    return value.replace("~", "~0").replace("/", "~1").replace("{", "%7B").replace("}", "%7D")
  }

  private fun normalizeOperationRef(ref: String): String {
    return ref.replace("{", "%7B").replace("}", "%7D").replace("%7b", "%7B").replace("%7d", "%7D")
  }
  ...
  private fun validatePathTemplateCollisions(
    ...
    val normalized = pathKeys.groupBy { normalizePathTemplate(it) }.filter { it.value.size > 1 }
    normalized.forEach { (template, paths) ->
      issues +=
          OpenApiIssue(
              ...
              path = basePath,
              message =
                  "Paths share the same templated structure '$template': ${paths.joinToString(", ")}.")
    }
  }
  ...
  private companion object {
    ...
    private val KNOWN_JSON_SCHEMA_DIALECTS =
        setOf(
            JSON_SCHEMA_2020_12,
            JSON_SCHEMA_2019_09,
...
data class OpenApiIssue(val severity: OpenApiIssueSeverity, val path: String, val message: String)
